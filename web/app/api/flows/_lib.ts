...
import { GateError, ReleaseError } from '@openbooks/engine/src/flows/index.ts'
...
export function gateErrorResponse(e: unknown): NextResponse {
  // A release failure is a server-side defect with the cause and the remedy
  // in the message (decision NOT recorded, gate still pending) — a 500 with
  // that body intact, never a success and never a bare 'internal error'.
  if (e instanceof ReleaseError) {
    console.error('[flows] approval release failed:', e)
