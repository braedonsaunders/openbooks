...
const { POST: createAccount, PATCH: updateAccount } = await import('./route')
...
test('bank-account writes wait on a party rehome before acting or disclosing account revisions', { skip: !process.env.OPENBOOKS_DB_URL }, async () => {
  ...
  try {
    for (const operation of ['create', 'patch'] as const) {
      const partyId = randomUUID(), accountId = randomUUID()
      await withBypassContext(() => db.execute(sql`insert into parties(id,org_id,kind,display_name,subsidiary_id,is_active,custom) values (${partyId},${org.orgId},'vendor','Rehoming vendor',${org.subsidiaryId},true,'{}'::jsonb)`))
      if (operation === 'patch') await withBypassContext(() => db.execute(sql`insert into party_bank_accounts(id,org_id,party_id,bank_name,account_number_encrypted,account_last_four,approval_status,is_active,submitted_by,created_by,updated_by) values (${accountId},${org.orgId},${partyId},'Bank','opaque','1234','pending',false,${actor},${actor},${actor})`))
      await writer.query('begin'); await writer.query("select set_config('app.bypass_rls','on',true)")
      const pid = (await writer.query<{ pid: number }>('select pg_backend_pid() as pid')).rows[0]!.pid
      await writer.query('update parties set subsidiary_id=$1 where id=$2 and org_id=$3', [hidden, partyId, org.orgId])
      pending = withOrgContext(org.orgId, () => operation === 'create'
        ? createAccount(createRequest(partyId), { params: Promise.resolve({ id: partyId }) })
        : updateAccount(new Request(`http://bank.local/api/parties/${partyId}/bank-accounts?accountId=${accountId}`, { method: 'PATCH', body: JSON.stringify({ bankName: 'Changed Bank', changeReason: 'updated banking details', expectedUpdatedAt: '000000' }) }), { params: Promise.resolve({ id: partyId }) }))
      let blocked = false
      for (let n = 0; n < 200; n++) { blocked = !!(await pool.query('select 1 from pg_stat_activity where $1=any(pg_blocking_pids(pid))', [pid])).rowCount; if (blocked) break; await new Promise((resolve) => setTimeout(resolve, 10)) }
      assert.ok(blocked, `${operation} must lock and scope-check the party before account work`)
    ...
    await writer.query('rollback').catch(() => {}); await pending?.catch(() => {}); writer.release()
    state.user = null; await withBypassContext(() => dropScratchOrg(org.orgId))
  }
})
