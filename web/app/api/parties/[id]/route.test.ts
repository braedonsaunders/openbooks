...
const routeState: RouteState = {
  ...
  existing: {
    ...
    is_active: true,
    updated_at: new Date('2026-08-24T12:00:00.400001Z'),
    customer_hold: false,
    ...
    `
      ...
      const record = (query) => {
        ...
        state.calls.push({ text, values: (query.queryChunks ?? []).flatMap((chunk) => {
          if (typeof chunk === 'string' || typeof chunk === 'number' || typeof chunk === 'boolean' || chunk === null) return [chunk]
          if (Array.isArray(chunk.queryChunks)) return chunk.queryChunks.filter((nested) => typeof nested !== 'object' || nested === null)
          return []
        }) })
        if (text.includes('select p.display_name')) return Promise.resolve({ rows: [state.existing] })
        if (text.includes('update parties set')) return Promise.resolve({ rows: [{ id: '00000000-0000-4000-8000-00000000a401' }] })
...
const hooks = registerHooks({
  resolve(specifier, context, nextResolve) {
    ...
    if (specifier === '@/lib/api/json') {
      return { shortCircuit: true, format: 'module', url: 'mock:json' }
    }
...
test('customer role updates preserve an existing hold when hold fields are omitted', async () => {
...
test('vendor role updates preserve an existing hold when hold fields are omitted', async () => {
...
test('customer hold release without a change reason is refused before any write', async () => {
  ...
  assert.equal(routeState.calls.filter(({ text }) => text.includes('update parties set') || text.includes('insert into customer_roles')).length, 0)
})
...
test('vendor hold release without a change reason is refused before any write', async () => {
  ...
  assert.equal(response.status, 422)
  assert.equal(routeState.calls.filter(({ text }) => text.includes('update parties set') || text.includes('insert into vendor_roles')).length, 0)
})
