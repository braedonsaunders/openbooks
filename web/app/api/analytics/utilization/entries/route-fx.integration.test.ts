...
test('utilization entries drill translates labour cost to presentation', async () => {
...
test('utilization entries drill refuses by name on missing FX coverage', async () => {
  const { org } = await setup();
  try {
    const employee = randomUUID();
