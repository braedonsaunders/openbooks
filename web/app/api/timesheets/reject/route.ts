...
export async function POST(req: Request) {
  ...
  const before = await loadWeek(orgId, ownedEmployee, week)
  if (before.status !== 'submitted') return bad('Only a submitted week can be rejected')

  await setTimesheetWeekStatus(orgId, ownedEmployee, week, 'rejected', user.id, reason)
  ...
  return NextResponse.json(await loadWeek(orgId, ownedEmployee, week))
}
