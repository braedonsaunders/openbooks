 ...
 * (plain digits, bounded decimals), so its single null covers five situations
 * with five different remedies: too many decimals (round), a thousands
 * separator (remove it — this is what a spreadsheet paste looks like), a
...
function decimalNullCause(raw: unknown): DecimalNullCause {
 ...
 * Names which of the five causes fired, the value received, and the remedy —
 * the same discipline the outer three-way split follows.
 */
function decimalNullRefusal(field: string, noun: string, raw: unknown, maxScale: number): string {
...
export async function POST(req: Request) {
  ...
  for (const key of MONEY_KEYS) {
    ...
    // The profile money columns are numeric(19,4), so four decimals are
    // accepted exactly as the column stores them — the old 2 was narrower
    // than the schema for no stated reason and refused storable values.
    const value = canonicalDecimal(raw, 4)
    ...
    // undiagnosable in the first place. The unreadable-value branch below is
    // itself split five ways, for the same reason one level down.
    if (value === null) {
      return NextResponse.json(
        { error: decimalNullRefusal(key, 'an amount', body[key], 4) },
        { status: 422 },
      )
    }
