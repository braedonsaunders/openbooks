    ...
    if (specifier.endsWith("/lib/authz") && context.parentURL?.includes("/api/admin/setup/project-types/")) {
      return {
        ...
        url:
          ...
          encodeURIComponent(
            `export { guardUnrestrictedScope } from '${root}web/lib/authz.ts';export async function guardPermission(){return {user:globalThis.__projectTypeFlagsUser.user,permissions:new Set(['*']),allowedSubsidiaryIds:null}}`,
          ),
      };
    }
