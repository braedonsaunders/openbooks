...
export async function POST(request: Request, { params }: { params: Promise<{ key: string }> }) {
  ...
  try {
    const result = await runExtensionAction(applicationContextFromSession(gate, 'api', crypto.randomUUID()), key, parsed.data)
    return NextResponse.json(result, { status: result.ok ? 200 : result.status })
  } catch (error) {
    if (error instanceof ApplicationError) return NextResponse.json({ ok: false, error: error.message }, { status: error.status })
    throw error
  }
}
