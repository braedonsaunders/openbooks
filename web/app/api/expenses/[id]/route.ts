...
/** Autosave for draft expense reports: header fields and/or full line replacement. */
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (existing.rows[0].status !== 'draft') {
    return NextResponse.json({ error: 'only draft expense reports can be edited' }, { status: 422 })
  }
