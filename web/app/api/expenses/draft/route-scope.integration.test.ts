...
const stateKey = Symbol.for('openbooks.expense-draft-scope-test')
interface Gate {
  ...
  resolve(specifier, context, nextResolve) {
    ...
    if (specifier === '../../../../lib/authz' ||
      ...
      return module_(`
        ...
        const state = globalThis[Symbol.for('openbooks.expense-draft-scope-test')];
...
async function setup() {
  ...
  const actor = await withBypassContext(() => (createScratchUser(org.orgId, 'Expense drafter', 'reviewer')))
  ...
  const gate = (scope: Set<string> | null) => {
    routeState.gate = {
      ...
      permissions: new Set(['expenses.create']),
...
async function expenseDrafts(orgId: string) {
  return (await db.execute<{ id: string; subsidiary_id: string | null }>(sql`
    select id, subsidiary_id from documents
     where org_id = ${orgId} and kind = 'expense_report'
  `)).rows
}

test('a restricted expense draft lands in their own subsidiary', { skip: !DB }, async () => {
...
test('an expense draft with no assignable subsidiary refuses by name and stores nothing', { skip: !DB }, async () => {
