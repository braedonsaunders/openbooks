 ...
 */
export function ArCockpit({ data, canCollect }: { data: ArPosition; canCollect: boolean }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="grid shrink-0 grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        <StatTile icon={Wallet} accent="sky" label={t('stats.openReceivables')} value={moneyCompact(data.outstanding)} />
        ...
        <StatTile icon={CalendarClock} accent="emerald" label={t('stats.expectedThisWeek')} value={moneyCompact(data.expectedThisWeek)} tone="positive" />
        <StatTile icon={CalendarRange} accent="teal" label={t('stats.next30')} value={moneyCompact(data.expectedNext30)} />
        <StatTile icon={Timer} accent="violet" label={t('stats.dso')} value={t('stats.days', { n: data.dso })} sub={t('stats.dsoSub')} />
      </div>

      {/* Worklist + right column — fill remaining height */}
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-5 lg:grid-cols-3">
        <CockpitPanel title={t('panels.worklist')} icon={ListChecks} actions={gear} bodyClassName="min-h-0 overflow-hidden p-0" className="min-h-0 lg:col-span-2">
          <CollectionsWorklist
        ...
        <div className="flex min-h-0 flex-col gap-5">
          <CockpitPanel title={t('panels.aging')} icon={ListOrdered} hint={`${moneyCompact(data.outstanding)} · ${data.summary.pctCurrent.toFixed(0)}% ${t('current')}`} className="shrink-0">
            <AgingBars buckets={data.summary.buckets} accent="text-sky-600 dark:text-sky-400" />
          </CockpitPanel>
          ...
          <CockpitPanel title={t('panels.byCustomer')} icon={Users} bodyClassName="min-h-0 overflow-hidden p-0" className="min-h-0 flex-1">
            <div className="h-full overflow-y-auto">
              {data.byCustomer.length === 0 ? (
                <p className="px-4 py-10 text-center text-sm text-slate-400 dark:text-slate-500">{t('noReceivables')}</p>
              ) : (
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {data.byCustomer.map((c) => (
                      <tr key={c.partyId ?? c.partyName} className="border-b border-slate-50 last:border-0 dark:border-slate-800/60">
                        <td className="px-4 py-2 text-slate-700 dark:text-slate-300">{c.partyName}</td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {c.overdue > 0 ? <span className="text-red-600 dark:text-red-400">{moneyCompact(c.overdue)}</span> : <span className="text-slate-300 dark:text-slate-600">—</span>}
                        </td>
                        <td className="px-4 py-2 text-right font-medium tabular-nums text-slate-800 dark:text-slate-200">{money(c.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
