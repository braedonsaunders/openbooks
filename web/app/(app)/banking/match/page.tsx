...
export default async function MatchBankData({
  ...
  if (session) {
    ...
    const [stmt, stmtC, gl, glC, review, ex, exC] = (await Promise.all([
      ...
      db.execute(sql`select count(*) as n from bank_statement_lines l join bank_statements s on s.id = l.statement_id where ${stmtWhere}`),
      db.execute(sql`
      ...
      db.execute(sql`select count(*) as n from journal_lines jl join journal_entries je on je.id = jl.entry_id where ${glWhere}`),
      ...
      db.execute(sql`select count(*) as n from bank_statement_lines l join bank_statements s on s.id = l.statement_id where ${exWhere}`),
    ])) as unknown as { rows: any[] }[]
