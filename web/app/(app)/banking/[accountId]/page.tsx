...
import { formatInZone } from '@openbooks/engine/src/business-date.ts'
...
export default async function BankingAccount({
  ...
  // Keep timestamp rendering aligned with the business-date machinery: the
  // organization's configured time zone is stored in org settings, with UTC
  // as the same safe fallback used by businessToday when it is absent/invalid.
  const orgTimeZoneRes = await db.execute<{ time_zone: string | null }>(sql`
    select settings->>'timeZone' as time_zone from orgs where id = ${orgId}
  `)
  const configuredTimeZone = orgTimeZoneRes.rows[0]?.time_zone?.trim()
  const timeZone = configuredTimeZone
    ? (() => {
        try {
          formatInZone(new Date(), configuredTimeZone)
