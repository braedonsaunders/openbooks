...
import type { JournalData } from './view'
...
const { journalSpec } = await import('./view')

// F-t06-027: enabling Multi-subsidiary crashed the journal report with React
...
// interface.)
function blockedData(): JournalData {
  return {
    ratesBlocked: {
...
test('a rates refusal renders as a derive banner gated on the block', () => {
  const blocks = blocksOf(journalSpec(blockedData()))
