...
// error 441 — 'No consolidated exchange rates … Derive rates from period
// close first.' threw out of SSR. The loader must convert the typed rates
// refusal into a banner with a derive link and never throw it.
...
for (const locale of ['en', 'fr', 'es']) {
  test(`the banner copy is translated in ${locale} (F-t06-025)`, () => {
    const messages = JSON.parse(
      readFileSync(new URL(`../../../../messages/${locale}/reports.json`, import.meta.url), 'utf8'),
    ) as { statement: Record<string, string> }
    for (const key of ['ratesBlockedTitle', 'ratesBlockedAction']) {
      assert.equal(typeof messages.statement[key], 'string', `${locale} must translate statement.${key}`)
      assert.ok(messages.statement[key]!.length > 0, `${locale} statement.${key} must not be empty`)
    }
  })
}
