...
export function ScheduleEditor({
  ...
  async function toggleActive(s: ScheduleRow) {
    ...
    else toast.success(s.active ? 'Schedule paused' : 'Schedule resumed')
    router.refresh()
  }

  async function remove(s: ScheduleRow) {
    const ok = await confirmDialog({ message: 'Delete this schedule?', tone: 'danger' })
    if (!ok) return
    ...
    <div className="space-y-3">
      {schedules.length === 0 && !adding ? (
        <div className="rounded-lg border border-dashed border-slate-300 px-4 py-6 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
          No delivery schedule yet. {canSchedule ? 'Add one to email this report on a cadence.' : ''}
        </div>
      ) : null}
          ...
          <div className="min-w-0 space-y-0.5">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">
              {s.recipient_emails.length > 0
                ...
                : 'No recipients set'}
              {' · Next run '}
              {String(s.next_run_at).slice(0, 16).replace('T', ' ')}
            </div>
          </div>
          {canSchedule ? (
            <div className="flex items-center gap-2">
...
function ScheduleForm({
  ...
  async function save() {
    ...
    if (!res.ok) {
      toast.error((await res.json()).error ?? 'Could not save schedule')
      setBusy(false)
      return
    }
    ...
    <div className="space-y-4 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
      ...
      <div className={field}>
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Comma-separated email addresses. Delivery is queued for the worker (see below).
        </p>
      </div>
      <div className="flex items-center gap-2">
