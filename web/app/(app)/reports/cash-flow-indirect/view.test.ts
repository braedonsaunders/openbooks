...
import type { CashFlowIndirectData } from './view'
...
// the page shows the banner and hides the paper. (The loader half needs a
// database and has no unit interface.)
function blockedData(): CashFlowIndirectData {
  return {
    ratesBlocked: {
    ...
    ratesReady: false,
  } as unknown as CashFlowIndirectData
}
...
test('a rates refusal renders as a derive banner gated on the block', () => {
  const blocks = blocksOf(cashFlowIndirectSpec(blockedData()))
