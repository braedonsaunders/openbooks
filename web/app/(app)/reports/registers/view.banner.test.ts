...
import type { RegistersData } from './view'
...
// banner and hides the paper. (The loader half needs a database and has no
// unit interface.)
function blockedData(): RegistersData {
  return {
    ratesBlocked: {
...
test('a rates refusal renders as a derive banner gated on the block', () => {
  const blocks = blocksOf(registersSpec(blockedData()))
