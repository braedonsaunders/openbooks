...
export function TaxPoolsView({ canRun }: { canRun: boolean }) {
  ...
  async function run() {
    ...
    try {
      const res = await fetch('/api/assets/tax-pools', {
        ...
        body: JSON.stringify({ regime: 'ca_cca', taxYear }),
      })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{t('taxPools.title')}</h1>
