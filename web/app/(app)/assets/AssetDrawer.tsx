...
export function AssetDrawer({
  ...
  const fieldId = (placement: HeaderFieldPlacement) => `${formId}-${placement.key}`
  ...
  const fieldLabel = (placement: HeaderFieldPlacement, fallback: string, required = false) => (
    <Label htmlFor={fieldId(placement)}>{labelFor(placement, fallback)}{editable && requiredFor(placement, required) ? <span className="text-red-500"> *</span> : null}</Label>
  )
  ...
  function renderAssetField(placement: HeaderFieldPlacement) {
    ...
    switch (key) {
      ...
      case 'acquisition_cost': return <>{fieldLabel(placement, t('labels.cost'), true)}{editable ? <Input id={fieldId(placement)} inputMode="decimal" className="text-right tabular-nums" value={cost} onChange={(e) => setCost(e.target.value)} /> : <p className="text-right text-sm tabular-nums">{money(cost)}</p>}</>
      ...
      case 'depreciation_units_total': return depreciationMethodId || method !== 'units_of_production' ? null : <>{fieldLabel(placement, t('labels.unitsTotal'))}{editable ? <Input id={fieldId(placement)} inputMode="decimal" className="text-right tabular-nums" value={unitsTotal} onChange={(e) => setUnitsTotal(e.target.value)} /> : <p className="text-right text-sm tabular-nums">{textValue(unitsTotal)}</p>}</>
      case 'depreciation_convention': return !depreciationMethodId && method === 'manual' ? null : <>{fieldLabel(placement, t('labels.convention'))}{editable ? <Select id={fieldId(placement)} value={convention} onChange={(e) => setConvention(e.target.value)}>{CONVENTIONS.map((item) => <option key={item} value={item}>{t(`conventions.${item}`)}</option>)}</Select> : <p className="text-sm">{t(`conventions.${convention}`)}</p>}</>
      case 'asset_account_id': return <>{fieldLabel(placement, t('labels.assetAccount'))}{editable ? <SearchSelect id={fieldId(placement)} value={assetAccountId} onChange={(value) => setAssetAccountId(value ?? '')} options={accountOptions} clearable placeholder={t('drawer.selectAccount')} ariaLabel={labelFor(placement, t('labels.assetAccount'))} /> : <p className="text-sm">{payload.accountNames.asset ?? '—'}</p>}</>
      case 'accumulated_depreciation_account_id': return <>{fieldLabel(placement, t('labels.accumulatedAccount'))}{editable ? <SearchSelect id={fieldId(placement)} value={accumAccountId} onChange={(value) => setAccumAccountId(value ?? '')} options={accountOptions} clearable placeholder={t('drawer.selectAccount')} ariaLabel={labelFor(placement, t('labels.accumulatedAccount'))} /> : <p className="text-sm">{payload.accountNames.accumulated ?? '—'}</p>}</>
      case 'depreciation_expense_account_id': return <>{fieldLabel(placement, t('labels.expenseAccount'))}{editable ? <SearchSelect id={fieldId(placement)} value={expenseAccountId} onChange={(value) => setExpenseAccountId(value ?? '')} options={accountOptions} clearable placeholder={t('drawer.selectAccount')} ariaLabel={labelFor(placement, t('labels.expenseAccount'))} /> : <p className="text-sm">{payload.accountNames.expense ?? '—'}</p>}</>
      default: return null
    }
  }
    ...
    {tab === 'tax' ? <div className="space-y-5 p-1">{taxConfigurations.map((config) => {
      ...
      return <section key={config.code} className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <div className="space-y-1.5"><Label htmlFor={`${formId}-tax-${config.code}-classCode`}>{t('drawer.taxClass')}</Label>{editable ? <Select id={`${formId}-tax-${config.code}-classCode`} value={String(values.classCode ?? '')} onChange={(event) => setTaxValue(config.code, 'classCode', event.target.value)}><option value="">{t('drawer.useCategoryDefault')}</option>{config.classes.map((item) => <option key={item.code} value={item.code}>{item.code} · {item.name}</option>)}</Select> : <p className="text-sm">{String(values.classCode ?? categoryDefault ?? '—')}</p>}</div>
          <div className="space-y-1.5"><Label htmlFor={`${formId}-tax-${config.code}-businessUsePercent`}>{t('drawer.businessUsePercent')}</Label>{editable ? <Input id={`${formId}-tax-${config.code}-businessUsePercent`} inputMode="decimal" value={String(values.businessUsePercent ?? 100)} onChange={(event) => setTaxValue(config.code, 'businessUsePercent', event.target.value)} /> : <p className="text-sm tabular-nums">{String(values.businessUsePercent ?? 100)}%</p>}</div>
          {config.code === 'us_macrs' ? <>
            <div className="space-y-1.5"><Label htmlFor={`${formId}-tax-${config.code}-section179`}>{t('drawer.section179')}</Label>{editable ? <Input id={`${formId}-tax-${config.code}-section179`} inputMode="decimal" value={String(values.section179 ?? 0)} onChange={(event) => setTaxValue(config.code, 'section179', event.target.value)} /> : <p className="text-sm tabular-nums">{money(String(values.section179 ?? 0))}</p>}</div>
            <div className="space-y-1.5"><Label htmlFor={`${formId}-tax-${config.code}-bonusPercent`}>{t('drawer.bonusPercent')}</Label>{editable ? <Input id={`${formId}-tax-${config.code}-bonusPercent`} inputMode="decimal" value={String(values.bonusPercent ?? 0)} onChange={(event) => setTaxValue(config.code, 'bonusPercent', event.target.value)} /> : <p className="text-sm tabular-nums">{String(values.bonusPercent ?? 0)}%</p>}</div>
          </> : null}
        </div>
      </section>
    })}</div> : null}
