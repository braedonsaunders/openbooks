...
/**
 * Vendor bills + credits. The list (search, type/status filters, saved views,
 * sortable typed table, drill-through, pagination) is the universal
 * RecordListView; this page owns the header, the New button, and the
 * ?doc= document flyout with its form-layout resolution.
 */
  ...
  return (
    <ListPageLayout header={<PageHeader title={t('list.title')} description={t('list.description')} actions={newButton} />}>
      <RecordListView
        ...
        renderRowActions={(row) => <DocumentRowActions id={row.id} status={row.status} config={DOC_KINDS[row.kind]} openHref={`/ap?doc=${row.id}`} />}
      />
    </ListPageLayout>
  )
}
