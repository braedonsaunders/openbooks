          ...
          {['queued', 'extracting'].includes(status) ? <div className="flex h-full min-h-72 items-center justify-center gap-3 text-slate-500"><Loader2 className="animate-spin" />{t('processing')}</div> : status === 'failed' ? <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">{t('processingFailed')}</div> : (
            <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1.5"><Label>{t('fields.kind')}</Label><Select value={documentKind} disabled={!editable} onChange={(event) => { setDocumentKind(event.target.value === 'vendor_credit' ? 'vendor_credit' : 'vendor_bill'); setDirty(true) }}><option value="vendor_bill">{t('bill')}</option><option value="vendor_credit">{t('credit')}</option></Select></div>
                <div className="space-y-1.5"><div className="flex items-center justify-between"><Label>{t('fields.vendor')}</Label>{confidence('vendorName')}</div><SearchSelect value={vendorId} onChange={(value) => { setVendorId(value); setDirty(true) }} options={options(vendors)} disabled={!editable} clearable placeholder={t('fields.vendorPlaceholder')} sheetTitle={t('fields.vendor')} ariaLabel={t('fields.vendor')} /></div>
                <Field label={t('fields.invoiceNumber')} badge={confidence('invoiceNumber')}><Input value={form.invoiceNumber ?? ''} disabled={!editable} onChange={(event) => update({ ...form, invoiceNumber: event.target.value })} /></Field>
                <Field label={t('fields.invoiceDate')} badge={confidence('invoiceDate')}><Input type="date" value={form.invoiceDate ?? ''} disabled={!editable} onChange={(event) => update({ ...form, invoiceDate: event.target.value })} /></Field>
                <Field label={t('fields.dueDate')} badge={confidence('dueDate')}><Input type="date" value={form.dueDate ?? ''} disabled={!editable} onChange={(event) => update({ ...form, dueDate: event.target.value || null })} /></Field>
                <Field label={t('fields.currency')} badge={confidence('currency')}><Input value={form.currency ?? ''} maxLength={3} disabled={!editable} onChange={(event) => update({ ...form, currency: event.target.value.toUpperCase() })} /></Field>
                <div className="space-y-1.5 sm:col-span-2"><div className="flex items-center justify-between"><Label>{t('fields.purchaseOrder')}</Label>{confidence('purchaseOrderNumber')}</div><SearchSelect value={purchaseOrderId} onChange={(value) => { setPurchaseOrderId(value); setDirty(true) }} options={options(purchaseOrders)} disabled={!editable} clearable placeholder={t('fields.purchaseOrderPlaceholder')} sheetTitle={t('fields.purchaseOrder')} ariaLabel={t('fields.purchaseOrder')} /></div>
                <div className="space-y-1.5 sm:col-span-2"><Label>{t('fields.memo')}</Label><Textarea value={form.memo ?? ''} disabled={!editable} onChange={(event) => update({ ...form, memo: event.target.value || null })} rows={2} /></div>
              </div>
              <div className="space-y-2">
                ...
                <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-800"><Table><TableHeader><TableRow><TableHead>{t('lines.description')}</TableHead><TableHead>{t('lines.account')}</TableHead><TableHead className="w-24">{t('lines.quantity')}</TableHead><TableHead className="w-28">{t('lines.unitPrice')}</TableHead><TableHead className="w-28">{t('lines.amount')}</TableHead><TableHead className="w-24">{t('lines.tax')}</TableHead>{editable ? <TableHead className="w-10"><span className="sr-only">{tc('actions.delete')}</span></TableHead> : null}</TableRow></TableHeader><TableBody>{form.lines.map((line, lineIndex) => <TableRow key={lineIndex}><TableCell className="min-w-44"><div className="mb-1 flex justify-end">{confidence('lines.description', lineIndex) ?? confidence('lines.amount', lineIndex)}</div><Input value={line.description} disabled={!editable} onChange={(event) => update({ ...form, lines: form.lines.map((value, index) => index === lineIndex ? { ...value, description: event.target.value } : value) })} /></TableCell><TableCell className="min-w-48"><SearchSelect value={line.accountId ?? ''} onChange={(value) => update({ ...form, lines: form.lines.map((current, index) => index === lineIndex ? { ...current, accountId: value || null, itemId: value ? null : current.itemId } : current) })} options={options(accounts)} disabled={!editable || Boolean(line.purchaseOrderLineId)} clearable placeholder={t('lines.accountPlaceholder')} sheetTitle={t('lines.account')} ariaLabel={t('lines.account')} /></TableCell>{(['quantity', 'unitPrice', 'amount', 'taxAmount'] as const).map((key) => <TableCell key={key}><Input inputMode="decimal" value={line[key]} disabled={!editable} onChange={(event) => update({ ...form, lines: form.lines.map((current, index) => index === lineIndex ? { ...current, [key]: event.target.value } : current) })} className="text-right tabular-nums" /></TableCell>)}{editable ? <TableCell><Button size="icon" variant="ghost" aria-label={t('lines.remove', { line: lineIndex + 1 })} onClick={() => update({ ...form, lines: form.lines.filter((_, index) => index !== lineIndex) })}><Trash2 size={14} /></Button></TableCell> : null}</TableRow>)}</TableBody></Table></div>
              </div>
              <div className="grid gap-3 sm:grid-cols-3"><Field label={t('fields.subtotal')} badge={confidence('subtotal')}><Input inputMode="decimal" value={form.subtotal ?? ''} disabled={!editable} onChange={(event) => update({ ...form, subtotal: event.target.value })} className="text-right tabular-nums" /></Field><Field label={t('fields.taxTotal')} badge={confidence('taxTotal')}><Input inputMode="decimal" value={form.taxTotal ?? ''} disabled={!editable} onChange={(event) => update({ ...form, taxTotal: event.target.value })} className="text-right tabular-nums" /></Field><Field label={t('fields.total')} badge={confidence('total')}><Input inputMode="decimal" value={form.total ?? ''} disabled={!editable} onChange={(event) => update({ ...form, total: event.target.value })} className="text-right font-semibold tabular-nums" /></Field></div>
            </div>
          )}
        </section>
      </div>
