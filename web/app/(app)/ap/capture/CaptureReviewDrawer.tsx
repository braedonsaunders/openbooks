...
import { Badge, Button, Input, SearchSelect, Select, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Textarea, UrlDrawer } from '@openbooks/ui'
          ...
          {['queued', 'extracting'].includes(status) ? <div className="flex h-full min-h-72 items-center justify-center gap-3 text-slate-500"><Loader2 className="animate-spin" />{t('processing')}</div> : status === 'failed' ? <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">{t('processingFailed')}</div> : (
            <div className="space-y-5">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Field label={t('fields.vendor')} aside={confidence('vendorName')}>{editable ? <SearchSelect value={vendorId} onChange={(value) => { setVendorId(value); markEdited() }} options={options(vendors)} clearable placeholder={t('fields.vendorPlaceholder')} sheetTitle={t('fields.vendor')} ariaLabel={t('fields.vendor')} /> : <ReadOnlyValue value={optionLabel(vendors, vendorId) || initial.resolvedVendor} />}</Field>
                <Field label={t('fields.invoiceNumber')} aside={confidence('invoiceNumber')}>{editable ? <Input value={form.invoiceNumber ?? ''} onChange={(event) => update({ ...form, invoiceNumber: event.target.value })} /> : <ReadOnlyValue value={form.invoiceNumber} className="font-mono" />}</Field>
                <Field label={t('fields.invoiceDate')} aside={confidence('invoiceDate')}>{editable ? <Input type="date" value={form.invoiceDate ?? ''} onChange={(event) => update({ ...form, invoiceDate: event.target.value })} /> : <ReadOnlyValue value={form.invoiceDate} />}</Field>
                <Field label={t('fields.dueDate')} aside={confidence('dueDate')}>{editable ? <Input type="date" value={form.dueDate ?? ''} onChange={(event) => update({ ...form, dueDate: event.target.value || null })} /> : <ReadOnlyValue value={form.dueDate} />}</Field>
                <Field label={t('fields.currency')} aside={confidence('currency')}>{editable ? <Input value={form.currency ?? ''} maxLength={3} onChange={(event) => update({ ...form, currency: event.target.value.toUpperCase() })} /> : <ReadOnlyValue value={form.currency} className="font-mono" />}</Field>
                <Field label={t('fields.purchaseOrder')} aside={confidence('purchaseOrderNumber')} className="space-y-1.5 sm:col-span-2">{editable && canLookupPurchaseOrders ? <SearchSelect value={purchaseOrderId} onChange={(value) => { setPurchaseOrderId(value); markEdited() }} options={options(purchaseOrders)} clearable placeholder={t('fields.purchaseOrderPlaceholder')} sheetTitle={t('fields.purchaseOrder')} ariaLabel={t('fields.purchaseOrder')} /> : <ReadOnlyValue value={optionLabel(purchaseOrders, purchaseOrderId) || initial.purchaseOrderNumber} />}</Field>
                <Field label={t('fields.memo')} className="space-y-1.5 sm:col-span-2">{editable ? <Textarea value={form.memo ?? ''} onChange={(event) => update({ ...form, memo: event.target.value || null })} rows={2} /> : <ReadOnlyValue value={form.memo} className="whitespace-pre-wrap" />}</Field>
              </div>
