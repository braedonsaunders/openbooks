...
function SovSection({ detail, accounts, canCreate, busy, act, money }: { detail: Detail; accounts: Option[]; canCreate: boolean; busy: boolean; act: Action; money: Money }) {
  ...
  const total = detail.sovLines.reduce((n, line) => n + Number(line.scheduledValue), 0);
...
function RetainageSection({ detail, canPost, busy, act, money }: { detail: Detail; canPost: boolean; busy: boolean; act: Action; money: Money }) {
  ...
  const postedHeld = detail.payApplications.filter((app) => app.vendorBillStatus === "posted").reduce((n, app) => n + Number(app.retainageThisPeriod), 0);
  const released = detail.retainageReleases.filter((release) => release.vendorBillStatus !== "voided").reduce((n, release) => n + Number(release.amount), 0);
