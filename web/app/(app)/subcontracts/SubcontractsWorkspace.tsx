...
type Subcontract = Summary & {
  ...
  endsOn: string | null;
  /** Optimistic-concurrency token: updated_at exactly as the loader read it. */
  updatedAt: string;
};
type SovLine = {
  id: string;
...
function Overview({ detail, money, canEdit, busy, act }: { detail: Detail; money: Money; canEdit: boolean; busy: boolean; act: Action }) {
  ...
  // Seeded once: the call site keys this section on the subcontract's
  // updated_at, so a 409 reload (which restamps it) remounts with the
  // other editor's values instead of saving stale terms over them.
...
function PayApplicationEditor({ app, lines, busy, act, currency, money }: { app: PayApplication; lines: PayApplicationLine[]; busy: boolean; act: Action; currency: string; money: Money }) {
  const t = useTranslations("subcontracts");
  const tCommon = useTranslations("common");
  // Seeded once: the call site keys this editor on the pay-application
  // revision, so a 409 reload (which bumps the revision) remounts with the
  // other editor's values instead of saving stale numbers over them.
