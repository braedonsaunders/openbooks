...
function StatusBadge({ status }: { status: string }) {
  ...
  return <Badge variant={tone as any}>{status.replaceAll("_", " ")}</Badge>;
}
...
function Overview({ detail, money, canEdit, busy, act }: { detail: Detail; money: (value: any, currency?: string) => string; canEdit: boolean; busy: boolean; act: any }) {
  const s = detail.subcontract;
  const billed = detail.payApplications.filter((app) => app.status === "billed").reduce((n, app) => n + Number(app.grossThisPeriod), 0);
...
function LifecycleActions({ detail, permissions, busy, act }: { detail: Detail; permissions: Permissions; busy: boolean; act: any }) {
  const s = detail.subcontract;
...
function SovSection({ detail, accounts, canCreate, busy, act, money }: any) {
...
function ControlsSection({ detail, parties, canPay, busy, act, money }: any) {
  ...
  return <div className="space-y-4">
    ...
    <Table><TableHeader><TableRow><TableHead>Control</TableHead><TableHead>Reason / joint payee</TableHead><TableHead>Effective</TableHead><TableHead>Ordinary-payment cap</TableHead><TableHead>Status</TableHead><TableHead /></TableRow></TableHeader><TableBody>{detail.paymentControls.map((c: any) => <TableRow key={c.id}><TableCell>{c.controlType.replaceAll("_", " ")}</TableCell><TableCell>{c.reason}{c.jointPayeeName ? ` · ${c.jointPayeeName}` : ""}</TableCell><TableCell>{c.effectiveOn}{c.expiresOn ? ` – ${c.expiresOn}` : ""}</TableCell><TableCell>{c.amountLimit ? money(c.amountLimit, detail.subcontract.currency) : "None (block all)"}</TableCell><TableCell><StatusBadge status={c.status} /></TableCell><TableCell>{c.status === "active" && canPay ? <Button size="sm" variant="outline" disabled={busy} onClick={() => { setReleaseId(c.id); setReleaseReason(""); }}>Release</Button> : null}</TableCell></TableRow>)}</TableBody></Table>
