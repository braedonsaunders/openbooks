 ...
 * `filter-chips` for status segments, the primary action is the shared
...
test('the performance list composes the shared table block with filter chips', () => {
  assert.match(view, /table\(\{\s*variant: 'app'/, 'cycles render through the shared table block')
  assert.match(view, /widgetBlock\('filter-chips'/, 'status segments use the shared filter-chips widget')
...
test('drawers open from URL search params through client islands', () => {
  ...
  assert.match(sections, /RetentionPanel/, 'the retention panel renders for HR')
})
