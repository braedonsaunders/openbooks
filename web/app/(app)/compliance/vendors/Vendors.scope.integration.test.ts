...
test('the vendor matrix hides vendors outside the caller scope', async () => {
...
test('vendor certificates hide another entity vendor records', async () => {
  const { org, hiddenVendor } = await seedVendors()
  try {
    const scope = new Set([org.subsidiaryId])
...
test('vendor waivers hide another entity vendor rows', async () => {
  const { org, hiddenVendor } = await seedVendors()
  try {
    const scope = new Set([org.subsidiaryId])
    assert.equal(
