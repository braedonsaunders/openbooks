    ...
    <div className="space-y-6">
      ...
      <Card className="p-4">
        ...
        <div className="grid gap-3 md:grid-cols-3 lg:grid-cols-4">
          ...
          <Field label="Published version"><Select value={lifecycleForm.planVersionId} onChange={(e) => setLifecycleForm({ ...lifecycleForm, planVersionId: e.target.value })}><option value="">Choose…</option>{eligibleVersions.map((v) => <option key={v.id} value={v.id}>{v.name} · v{v.versionNumber}</option>)}</Select></Field>
          <Field label="Term starts"><Input type="date" value={lifecycleForm.termStartsOn} onChange={(e) => setLifecycleForm({ ...lifecycleForm, termStartsOn: e.target.value })} /></Field>
          <Field label="Term ends"><Input type="date" value={lifecycleForm.termEndsOn} onChange={(e) => setLifecycleForm({ ...lifecycleForm, termEndsOn: e.target.value })} /></Field>
          <Field label="Trial ends"><Input type="date" value={lifecycleForm.trialEndsOn} onChange={(e) => setLifecycleForm({ ...lifecycleForm, trialEndsOn: e.target.value })} /></Field>
          <Field label="Renewal"><Select value={lifecycleForm.renewalPolicy} onChange={(e) => setLifecycleForm({ ...lifecycleForm, renewalPolicy: e.target.value })}><option value="auto">Auto-renew</option><option value="manual">Manual renewal</option><option value="none">No renewal</option></Select></Field>
          <Field label="Renewal term (months)"><Input type="number" value={lifecycleForm.renewalTermMonths} onChange={(e) => setLifecycleForm({ ...lifecycleForm, renewalTermMonths: e.target.value })} /></Field>
      ...
      <Card className="p-4">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <Field label="Subscription"><Select value={amendForm.subscriptionId} onChange={(e) => setAmendForm({ ...amendForm, subscriptionId: e.target.value })}><option value="">Choose…</option>{subscriptions.filter((s) => lifecycleIds.has(s.id)).map((s) => <option key={s.id} value={s.id}>{s.customerName ?? "Customer"} · {s.planName}</option>)}</Select></Field>
          <Field label="Change"><Select value={amendForm.type} onChange={(e) => setAmendForm({ ...amendForm, type: e.target.value })}><option value="add_component">Add component</option><option value="change_component">Change component</option><option value="remove_component">Remove component</option><option value="change_term">Change term</option><option value="change_timing">Change billing timing</option><option value="renew">Renew</option><option value="coterm">Co-term</option></Select></Field>
          <Field label="Effective on"><Input type="date" value={amendForm.effectiveOn} onChange={(e) => setAmendForm({ ...amendForm, effectiveOn: e.target.value })} /></Field>
          <Field label="Reason"><Input value={amendForm.reason} onChange={(e) => setAmendForm({ ...amendForm, reason: e.target.value })} /></Field>
          {["add_component", "change_component", "remove_component"].includes(amendForm.type) && <><Field label="Component key"><Input value={amendForm.componentKey} onChange={(e) => setAmendForm({ ...amendForm, componentKey: e.target.value })} /></Field>{amendForm.type !== "remove_component" && <><Field label="Name"><Input value={amendForm.name} onChange={(e) => setAmendForm({ ...amendForm, name: e.target.value })} /></Field><Field label="Quantity"><Input type="number" value={amendForm.quantity} onChange={(e) => setAmendForm({ ...amendForm, quantity: e.target.value })} /></Field><Field label="Unit price"><Input type="number" value={amendForm.unitPrice} onChange={(e) => setAmendForm({ ...amendForm, unitPrice: e.target.value })} /></Field></>}</>}
          {amendForm.type === "change_term" && <Field label="New term end"><Input type="date" value={amendForm.termEndsOn} onChange={(e) => setAmendForm({ ...amendForm, termEndsOn: e.target.value })} /></Field>}
          {amendForm.type === "change_timing" && <Field label="Timing"><Select value={amendForm.billingTiming} onChange={(e) => setAmendForm({ ...amendForm, billingTiming: e.target.value })}><option value="advance">In advance</option><option value="arrears">In arrears</option></Select></Field>}
          {amendForm.type === "renew" && <Field label="Renewal months"><Input type="number" value={amendForm.renewalTermMonths} onChange={(e) => setAmendForm({ ...amendForm, renewalTermMonths: e.target.value })} /></Field>}
          {amendForm.type === "coterm" && <Field label="Anchor subscription"><Select value={amendForm.anchorSubscriptionId} onChange={(e) => setAmendForm({ ...amendForm, anchorSubscriptionId: e.target.value })}><option value="">Choose…</option>{subscriptions.filter((s) => s.id !== amendForm.subscriptionId && lifecycleIds.has(s.id) && (!amendmentSubscription || s.customerName === amendmentSubscription.customerName)).map((s) => <option key={s.id} value={s.id}>{s.planName}</option>)}</Select></Field>}
        </div>
