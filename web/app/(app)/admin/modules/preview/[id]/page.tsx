...
import { parseManifest, contentTypeFor } from '@/lib/apps/manifest'
...
export default async function ExtensionPreview({ params, searchParams }: {
  ...
  const t = await getTranslations('admin.modules.draft')
  ...
  const replacements = Object.fromEntries(bundle.files.filter(file => file.path !== entry.path && !manifest.endpoints.some(endpoint => endpoint.file === file.path) && !file.path.startsWith('objects/')).map(file => [file.path, `data:${contentTypeFor(file.path).contentType};base64,${file.isBinary ? file.content : Buffer.from(file.content).toString('base64')}`]))
  return <>{notice}<AppFrame appKey={manifest.key} context={{ app: { id, key: manifest.key, name: manifest.name }, user: { id: authz.user.id, name: authz.user.name, roles: authz.user.roles.map(role => role.key) } }} previewBundle={{ entry: entry.path, entryHtml: entry.content, replacements }} /></>
}
