...
export async function loadAdminExtensions(
  ...
  // /admin/extensions is a separate route segment from /apps, so the apps layout
  ...
  const appKey = pickString(sp.extension)
  ...
  return {
    ...
    drafts: drafts.map(draft => ({ ...draft, href: `/admin/extensions?draft=${draft.id}` })),
    ...
    rows: apps.rows.map((a) => ({
      key: String(a.key),
      name: String(a.name),
      href: buildListDrawerHref('/admin/extensions', sp, 'extension', String(a.key)),
    ...
    header: [
      pageHeader({
        ...
        actions: [
          ...
          ...(data.canAuthor ? [widget('link-button', { href: '/admin/extensions?new=1', label: data.newLabel, iconKey: 'plus' })] : []),
        ],
      }),
      grid('flex flex-wrap items-center gap-2', [
        widgetBlock('search-input', { placeholder: data.searchPlaceholder }),
        widgetBlock('filter-chips', {
          basePath: '/admin/extensions',
