...
export function ExtensionReview({ draft }: { draft: ExtensionDraft }) {
  ...
  async function activate(action: 'activate' | 'discard') {
    ...
    try {
      ...
      router.push(data.reviewUrl ?? '/admin/extensions'); router.refresh()
  ...
  async function previewPage(route: string) {
    setBusy(true); setError(null)
    try {
      const response = await fetch('/api/extensions/drafts', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'preview-page',draftId:draft.id,route})})
      const data = await response.json()
      if (!response.ok || !data.staged) throw new Error(data.error ?? data.errors?.join('; ') ?? t('failed'))
      router.push(data.previewUrl)
    } catch (e) { setError(e instanceof Error ? e.message : t('failed')) }
    finally { setBusy(false) }
  }
  return <UrlDrawer open closeHref="/admin/extensions" size="xl" title={manifest.name} description={`${t('title')} · ${manifest.version}`}>
    <div className="space-y-5">
      <p>{draft.reason}</p>
      <Alert variant="info"><AlertDescription>{t('unpublished')}</AlertDescription></Alert>
      <div className="flex gap-2"><Button asChild variant="outline"><Link href={`/admin/extensions/preview/${draft.id}` as never} target="_blank">{t('preview')}</Link></Button><Button asChild variant="outline"><Link href={`/assistant?q=${encodeURIComponent(`Revise extension draft ${draft.id}. Read it with get_extension_draft, ask what I want changed, and prepare a new draft for review. Do not activate it.`)}` as never}>{t('revise')}</Link></Button></div>
      ...
      <section><h3 className="font-semibold">{t('contents')}</h3><ul className="list-inside list-disc text-sm">{objects.recordTypes.map(type => <li key={type.key}>{t('recordType', { name: type.name })}</li>)}{objects.customFields.map(field => <li key={`${field.targetTable}:${field.key}`}>{t('field', { name: field.label })}</li>)}{manifest.endpoints.map(endpoint => <li key={endpoint.name}>{t('action', { name: endpoint.name })}</li>)}{(manifest.contributions ?? []).map((item,index) => <li key={`contribution:${index}`}>{t(item.kind, { name: item.kind === 'page' ? item.route : item.label })}</li>)}<li>{t('files', { count: draft.bundle.files.length })}</li></ul></section>
      {(manifest.contributions ?? []).filter(item=>item.kind==='page' && !item.route.includes('[')).map(item=>item.kind==='page' ? <Button key={item.route} variant="outline" disabled={busy} onClick={()=>previewPage(item.route)}>{t('previewPage',{route:item.route})}</Button> : null)}
