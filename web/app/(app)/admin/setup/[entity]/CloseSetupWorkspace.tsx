...
function PeriodsTab(props: Props) {
  ...
  return <div className="space-y-5"><SectionHeading icon={<LockKeyhole size={18} />} title={t("periods.title")} description={t("periods.description")} action={<Button size="sm" asChild><Link href={drawerHref(props.currentParams, "periods", "period", "new") as any}><Plus size={14} />{t("actions.generate")}</Link></Button>} />
    <div className="flex flex-wrap items-end justify-between gap-3"><div className="min-w-64 max-w-md flex-1"><SearchInput paramKey="periodQ" pageParamKey="periodPage" placeholder={t("periods.searchPlaceholder")} /></div><div className="flex flex-wrap items-end gap-2"><Field label={t("fields.book")}>{props.books.length > 1 ? <Select className="h-9 w-44" value={props.selectedBookId} onChange={(e) => router.push(mergeHref(BASE, props.currentParams, { tab: "periods", book: e.target.value, periodPage: 1 }) as any)}>{props.books.map((book) => <option key={book.id} value={book.id}>{book.name}</option>)}</Select> : <div className="flex h-9 w-44 items-center rounded-md border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200">{props.books[0]?.name ?? "—"}</div>}</Field><Button variant="outline" size="sm" asChild><Link href="/admin/setup/accounting-books">{t("actions.manageBooks")}</Link></Button><Field label={t("fields.fiscalYear")}><Input className="h-9 w-28" type="number" value={props.fiscalYear} onChange={(e) => router.push(mergeHref(BASE, props.currentParams, { tab: "periods", fy: e.target.value, periodPage: 1 }) as any)} /></Field></div></div>
