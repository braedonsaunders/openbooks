 ...
 * own shape below — column types per the schema (uuids as strings, `date`
 * columns as ISO-date strings, counts as driver strings, JSONB parsed).
 * Drafts are the working copies the drawers edit before saving.
 */
...
export interface CalendarOptionRow extends Record<string, unknown> {
  id: string
  name: string
  is_default: boolean
  is_active: boolean
}
export interface PeriodLockState extends Record<string, unknown> {
  state: string
  lockedAt: string | null
  reason: string | null
  reopenExpiresAt: string | null
}
export interface PeriodRow extends Record<string, unknown> {
  id: string
  name: string
  starts_on: string
  ends_on: string
  ...
  locks: Record<string, PeriodLockState>
  entries: string
}
export interface CloseSetupBookRow extends Record<string, unknown> {
  id: string
  code: string
  name: string
  is_primary: boolean
}
export interface BlueprintStepRow extends Record<string, unknown> {
  id: string
  blueprint_id: string
  key: string
  title: string
...
export interface BlueprintBaseRow extends Record<string, unknown> {
  id: string
  name: string
  description: string | null
  period_type: string
...
export interface BlueprintRow extends BlueprintBaseRow {
...
export interface SetupUserRow extends Record<string, unknown> { id: string; name: string; email: string }
export interface SetupRoleRow extends Record<string, unknown> { key: string; name: string }
export interface ReportDefEntry extends Record<string, unknown> {
  slug: string
  name: string
  kind: string
  descriptor: ReportDescriptor
}
export interface ReopenRow extends Record<string, unknown> {
  id: string
  period_id: string
  book_id: string
  status: string
...
/** Working copies the drawers edit (camelCase) before saving. */
export interface CalendarDraft extends Record<string, unknown> {
  id?: string
  name: string
  cadence: string
  ...
  weekStartsOn: number
  timeZone: string
  adjustmentPeriodEnabled: boolean
  isDefault: boolean
  isActive: boolean
  anchorDate: string
  configText: string
}
export interface BlueprintStepDraft extends Record<string, unknown> {
  key: string
  title: string
  description: string
  workstream: string
  ...
  defaultReviewerRoleKey: string
  applicabilityText: string
  dependsOn: string[]
}
export interface BlueprintDraft extends Record<string, unknown> {
  id?: string
  name: string
  description: string
  periodType: string
...
export interface PolicyDraft extends Record<string, unknown> {
  id?: string
  code: string
  name: string
  description: string
...
export interface AutomationDraft extends Record<string, unknown> {
  id?: string
  name: string
  trigger: string
...
export interface PackageDraft extends Record<string, unknown> {
  id?: string
  name: string
  description: string
...
type Props = {
  ...
  calendarOptions: CalendarOptionRow[];
  periods: PeriodRow[];
  books: CloseSetupBookRow[];
...
function KeyValueRows({ rows, onChange }: { rows: Array<{ key: string; value: string }>; onChange: (next: Array<{ key: string; value: string }>) => void }) {
  const t = useTranslations("close.setup");
  return <div className="space-y-2">{rows.map((row, index) => <div key={index} className="flex items-center gap-2"><Input className="flex-1" placeholder={t("kv.key")} aria-label={`${t("kv.key")} ${index + 1}`} value={row.key} onChange={(event) => onChange(rows.map((item, i) => i === index ? { ...item, key: event.target.value } : item))} /><Input className="flex-1" placeholder={t("kv.value")} aria-label={`${t("kv.value")} ${index + 1}`} value={row.value} onChange={(event) => onChange(rows.map((item, i) => i === index ? { ...item, value: event.target.value } : item))} /><Button variant="ghost" size="sm" aria-label={t("actions.removeValue", { value: row.key.trim() || String(index + 1) })} onClick={() => onChange(rows.filter((_, i) => i !== index))}><Trash2 size={14} /></Button></div>)}<Button variant="outline" size="sm" onClick={() => onChange([...rows, { key: "", value: "" }])}><Plus size={14} />{t("kv.add")}</Button></div>;
}
...
function CalendarDrawer({ row, params }: { row?: CalendarRow; params: Props["currentParams"] }) {
...
function PeriodDrawer({ row, props }: { row?: PeriodRow; props: Props }) {
  const { date } = useViewerFormat();
  const t = useTranslations("close.setup"); const td = useTranslations("close"); const router = useRouter(); const closeHref = drawerHref(props.currentParams, "periods", "period", null); const creating = pickString(props.currentParams.period) === "new"; const selectedBook = props.books.find((book) => book.id === props.selectedBookId) ?? props.books[0]; const [calendarId, setCalendarId] = useState(props.calendarOptions.find((item) => item.is_default)?.id ?? props.calendarOptions[0]?.id ?? ""); const [year, setYear] = useState(props.fiscalYear); const [action, setAction] = useState<{ module: string; state: string } | null>(null); const [reason, setReason] = useState(""); const [busy, setBusy] = useState(false); const [generateError, setGenerateError] = useState<string | null>(null);
  async function generate() { setBusy(true); setGenerateError(null); try { await post({ action: "generate-periods", calendarId, fiscalYear: year }); toast.success(t("messages.periodsGenerated")); router.push((mergeHref(closeHref, {}, { fy: year }))); router.refresh(); } catch (error) { setGenerateError(error instanceof Error && error.message ? error.message : t("errors.actionFailed")); toast.error(t("errors.actionFailed")); } finally { setBusy(false); } }
  async function apply() { if (!row || !selectedBook || !action || !reason.trim()) return; setBusy(true); try { if (action.state === "reopen") await post({ action: "request-reopen", periodId: row.id, bookId: selectedBook.id, modules: [action.module], reason }); else await post({ action: "set-lock", periodId: row.id, bookId: selectedBook.id, module: action.module, state: action.state, reason }); toast.success(t(action.state === "reopen" ? "messages.reopenRequested" : action.state === "open" ? "messages.unlockSaved" : "messages.lockSaved")); setAction(null); setReason(""); router.refresh(); } catch { toast.error(t("errors.actionFailed")); } finally { setBusy(false); } }
  if (creating) return <UrlDrawer open closeHref={closeHref} size="md" title={t("periods.generateTitle")} description={t("periods.generateDescription")} footer={<div className="flex w-full justify-end gap-2"><Button variant="outline" onClick={() => router.push((closeHref))}>{t("actions.cancel")}</Button><Button disabled={busy || !calendarId} onClick={generate}>{busy ? t("actions.saving") : t("actions.generate")}</Button></div>}><div className="space-y-4 p-1">{generateError ? <p role="alert" className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">{generateError}</p> : null}<Field label={t("fields.calendar")}><Select value={calendarId} onChange={(e) => setCalendarId(e.target.value)}>{props.calendarOptions.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{dataText(td, item.name)}</option>)}</Select></Field><Field label={t("fields.fiscalYear")}><Input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} /></Field></div></UrlDrawer>;
  if (!row) return null;
  // Pending reopen requests for this period+book, from the already-loaded
  // setup-page list (F-t06-012): without them the drawer invites blind
  // duplicate requests and the requester sees no pending state.
  const pendingReopen = (props.reopenRequests ?? []).filter((item) => item.period_id === row.id && item.book_id === (selectedBook?.id ?? null) && item.status === "requested");
  return <UrlDrawer open closeHref={closeHref} size="lg" title={row.name} description={`${row.starts_on} → ${row.ends_on} · ${selectedBook?.name ?? ""}`}><div className="space-y-5 p-1"><div className="grid gap-2">{MODULES.map((module) => { const lock = row.locks?.[module]; const state = lock?.state ?? "open"; return <div key={module} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 p-3 dark:border-slate-800"><div><div className="flex items-center gap-2"><span className="font-medium">{t(`modules.${module}`)}</span><Badge variant={state === "closed" ? "success" : state === "soft_closed" ? "warning" : "outline"}>{t(`states.${state}`)}</Badge></div>{lock?.lockedAt ? <p className="mt-1 text-xs text-slate-500">{t("periods.lockedOn", { date: date(new Date(lock.lockedAt)) })}</p> : null}</div><div className="flex gap-1">{state === "open" ? <Button variant="outline" size="sm" onClick={() => setAction({ module, state: "soft_closed" })}>{t("actions.softClose")}</Button> : null}{state === "soft_closed" ? <Button variant="outline" size="sm" onClick={() => setAction({ module, state: "open" })}>{t("actions.unlock")}</Button> : null}<Button size="sm" variant={state === "closed" ? "outline" : "default"} disabled={state === "closed" && !props.canReopen} onClick={() => setAction({ module, state: state === "closed" ? "reopen" : "closed" })}>{state === "closed" ? t("actions.reopen") : t("actions.close")}</Button></div></div>; })}</div>{pendingReopen.length > 0 ? <div className="space-y-2"><h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{t("periods.reopenRequests")}</h4>{pendingReopen.map((item) => <Link key={item.id} href={mergeHref(BASE, props.currentParams, { tab: "periods", period: null, reopen: item.id })} className="block rounded-lg border border-amber-300 bg-amber-50/60 p-3 dark:border-amber-800 dark:bg-amber-950/20"><div className="flex flex-wrap items-center gap-2"><Badge variant="warning">{t("reopenStates.requested")}</Badge><span className="text-sm font-medium text-slate-900 dark:text-slate-100">{(item.modules ?? []).map((module) => t(`modules.${String(module)}`)).join(", ")}</span></div><p className="mt-1 text-sm text-slate-600 dark:text-slate-300">{item.reason}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("periods.requestedBy", { name: item.requester_name })}</p></Link>)}</div> : null}{action ? <div className="space-y-3 rounded-lg border border-teal-300 bg-teal-50/50 p-4 dark:border-teal-800 dark:bg-teal-950/20"><div><h4 className="font-medium">{action.state === "reopen" ? t("periods.reopenTitle") : action.state === "open" ? t("periods.unlockTitle") : t("periods.lockTitle")}</h4><p className="text-sm text-slate-500">{t("periods.lockDescription", { period: row.name, module: t(`modules.${action.module}`) })}</p></div><Field label={t("fields.reason")}><Textarea value={reason} onChange={(e) => setReason(e.target.value)} /></Field><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => { setAction(null); setReason(""); }}>{t("actions.cancel")}</Button><Button disabled={busy || !reason.trim()} onClick={apply}>{action.state === "reopen" ? t("actions.requestReopen") : action.state === "open" ? t("actions.unlock") : t("actions.confirmClose")}</Button></div></div> : null}</div></UrlDrawer>;
}
...
function PolicyDrawer({ row, params }: { row?: PolicyRow; params: Props["currentParams"] }) {
  ...
  async function save() {
    ...
    try {
      ...
      router.refresh();
    } catch (error) { toast.error(error instanceof Error && error.message ? error.message : t("errors.invalidConfiguration")); } finally { setBusy(false); }
  }
      ...
      <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
        {draft.policyType === "materiality" ? <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label={t("policyRules.percent")} hint={t("policyRules.percentHint")}><Input type="number" min={0} value={String(rules.percent ?? "")} placeholder="20" onChange={(e) => setRules({ percent: e.target.value === "" ? "" : Number(e.target.value) })} /></Field>
          </div>
        </> : null}
        {draft.policyType === "lock" ? <>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label={t("policyRules.defaultHours")}><Input type="number" min={1} value={String(rules.defaultHours ?? "")} placeholder="24" onChange={(e) => setRules({ defaultHours: e.target.value === "" ? "" : Number(e.target.value) })} /></Field>
            <Field label={t("policyRules.maxHours")}><Input type="number" min={1} value={String(rules.maxHours ?? "")} placeholder="168" onChange={(e) => setRules({ maxHours: e.target.value === "" ? "" : Number(e.target.value) })} /></Field>
          </div>
        </> : null}
...
function AutomationDrawer({ row, props }: { row?: AutomationRow; props: Props }) {
  const t = useTranslations("close.setup");
  const td = useTranslations("close");
  const router = useRouter();
  const closeHref = drawerHref(props.currentParams, "automation", "automation", null);
  const [draft, setDraft] = useState<AutomationDraft>(row
    ...
    <div className="space-y-4 p-1">
      ...
      <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Field label={t("conditions.minReadiness")}><Input type="number" min={0} max={100} value={String(conditions.minReadiness ?? "")} onChange={(e) => setCondNum("minReadiness", e.target.value)} /></Field>
          <Field label={t("conditions.maxReadiness")}><Input type="number" min={0} max={100} value={String(conditions.maxReadiness ?? "")} onChange={(e) => setCondNum("maxReadiness", e.target.value)} /></Field>
          <Field label={t("conditions.withinDays")}><Input type="number" value={String(conditions.withinDays ?? "")} onChange={(e) => setCondNum("withinDays", e.target.value)} /></Field>
        </div>
      </div>
      <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
        ...
        {action === "notify" ? <>
          ...
          <Field label={t("automationConfig.title")}><Input value={String(config.title ?? "")} placeholder={t("automationConfig.titlePlaceholder")} onChange={(e) => setConfig({ title: e.target.value })} /></Field>
          <Field label={t("automationConfig.body")}><Textarea value={String(config.body ?? "")} placeholder={t("automationConfig.bodyPlaceholder")} onChange={(e) => setConfig({ body: e.target.value })} /></Field>
        </> : null}
        {action === "assign" ? <>
          <p className="text-xs text-slate-500 dark:text-slate-400">{t("automationConfig.assignHint")}</p>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label={t("automationConfig.owner")}><SearchSelect value={String(config.ownerUserId ?? "")} onChange={(v) => setConfig({ ownerUserId: v })} options={userOptions} clearable searchable ariaLabel={t("automationConfig.owner")} /></Field>
            <Field label={t("automationConfig.ownerRole")}><SearchSelect value={String(config.ownerRoleKey ?? "")} onChange={(v) => setConfig({ ownerRoleKey: v })} options={roleOptions} clearable searchable ariaLabel={t("automationConfig.ownerRole")} /></Field>
            <Field label={t("automationConfig.reviewer")}><SearchSelect value={String(config.reviewerUserId ?? "")} onChange={(v) => setConfig({ reviewerUserId: v })} options={userOptions} clearable searchable ariaLabel={t("automationConfig.reviewer")} /></Field>
            <Field label={t("automationConfig.reviewerRole")}><SearchSelect value={String(config.reviewerRoleKey ?? "")} onChange={(v) => setConfig({ reviewerRoleKey: v })} options={roleOptions} clearable searchable ariaLabel={t("automationConfig.reviewerRole")} /></Field>
          </div>
        </> : null}
        {action === "create_task" ? <>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label={t("automationConfig.taskKey")}><Input value={String(config.key ?? "")} placeholder={t("automationConfig.taskKeyPlaceholder")} onChange={(e) => setConfig({ key: e.target.value })} /></Field>
            <Field label={t("automationConfig.taskTitle")}><Input value={String(config.title ?? "")} onChange={(e) => setConfig({ title: e.target.value })} /></Field>
            <Field label={t("fields.workstream")}><Select value={String(config.workstream ?? "review")} onChange={(e) => setConfig({ workstream: e.target.value })}>{ALL_WORKSTREAMS.map((value) => <option key={value} value={value}>{t(`workstreams.${value}`)}</option>)}</Select></Field>
            <Field label={t("automationConfig.gate")}><Select value={String(config.gateType ?? "none")} onChange={(e) => setConfig({ gateType: e.target.value })}>{["none", "soft", "hard"].map((value) => <option key={value} value={value}>{t(`gates.${value}`)}</option>)}</Select></Field>
          </div>
          <Field label={t("automationConfig.taskDescription")}><Textarea value={String(config.description ?? "")} onChange={(e) => setConfig({ description: e.target.value })} /></Field>
        ...
        {action === "generate_report" ? <>
          <Field label={t("automationConfig.report")}><SearchSelect value={String(config.report ?? "")} onChange={(v) => setConfig({ report: v })} options={reportOptions} searchable clearable ariaLabel={t("automationConfig.report")} emptyLabel={t("reportsEmpty")} /></Field>
          <Field label={t("automationConfig.label")}><Input value={String(config.label ?? "")} onChange={(e) => setConfig({ label: e.target.value })} /></Field>
        </> : null}
        {action === "start_flow" ? <>
          <div className="grid gap-3 sm:grid-cols-2">
            <Field label={t("automationConfig.subjectKind")}><Input value={String(config.subjectKind ?? "")} onChange={(e) => setConfig({ subjectKind: e.target.value })} /></Field>
            <Field label={t("automationConfig.subjectId")}><Select value={String(config.subjectId ?? "$run")} onChange={(e) => setConfig({ subjectId: e.target.value })}>{["$run", "$task"].map((value) => <option key={value} value={value}>{t(`subjectRefs.${value}`)}</option>)}</Select></Field>
          </div>
          <Field label={t("automationConfig.buttonId")}><Input value={String(config.buttonId ?? "")} onChange={(e) => setConfig({ buttonId: e.target.value })} /></Field>
        </> : null}
...
function ReportAttachmentCard({ attachment, meta, props, onChange, onRemove }: { attachment: ReportAttachment; meta?: ReportDefEntry; props: Props; onChange: (patch: Partial<ReportAttachment>) => void; onRemove: () => void }) {
  ...
  const dimSelect = (label: string, key: keyof ReportAttachment, options: Opt[]) =>
    <Field label={label}><SearchSelect value={String(attachment[key] ?? "")} onChange={(value) => onChange({ [key]: value || undefined } as Partial<ReportAttachment>)} options={options.map((option) => ({ value: option.id, label: option.name ?? option.id }))} clearable searchable ariaLabel={label} placeholder={t("reportParams.anyValue")} /></Field>;
...
function PackageDrawer({ row, props }: { row?: PackageRow; props: Props }) {
  const t = useTranslations("close.setup");
  const td = useTranslations("close");
  const router = useRouter();
  ...
  const [draft, setDraft] = useState<PackageDraft>(row
    ? { id: row.id, name: dataText(td, row.name), description: dataText(td, row.description) ?? "", reports: normalizeAttachments(row.reports), recipientsText: Array.isArray(row.recipients) ? row.recipients.join("\n") : "", delivery: row.delivery ?? {}, isDefault: row.is_default, isActive: row.is_active }
  ...
  return <UrlDrawer open closeHref={closeHref} size="lg" title={t("packages.formTitle")} footer={<div className="flex w-full justify-end gap-2"><Button variant="outline" onClick={() => router.push((closeHref))}>{t("actions.cancel")}</Button><Button disabled={busy || !draft.name} onClick={save}>{busy ? t("actions.saving") : t("actions.save")}</Button></div>}>
    <div className="space-y-4 p-1">
      ...
      <div className="space-y-3 rounded-lg border border-slate-200 p-4 dark:border-slate-800">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label={t("delivery.format")}><Select value={String(delivery.format ?? "pdf")} onChange={(e) => setDelivery({ format: e.target.value })}>{["pdf", "xlsx", "both"].map((value) => <option key={value} value={value}>{t(`delivery.formats.${value}`)}</option>)}</Select></Field>
          <Field label={t("delivery.cadence")}><Select value={String(delivery.cadence ?? "on_publish")} onChange={(e) => setDelivery({ cadence: e.target.value })}>{["on_publish", "manual"].map((value) => <option key={value} value={value}>{t(`delivery.cadences.${value}`)}</option>)}</Select></Field>
        </div>
