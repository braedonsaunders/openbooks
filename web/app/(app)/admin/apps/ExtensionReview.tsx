...
export function ExtensionReview({ draft }: { draft: ExtensionDraft }) {
  ...
  return (
    <UrlDrawer
      ...
      title={manifest.name}
      description={`${t('title')} · ${manifest.version}`}
    >
      <div className="space-y-5">
        {draft.status === 'draft' ? (
          ...
          >
            {t(editing ? 'backToReview' : 'editPackage')}
          </Button>
        ) : null}
        {draft.status === 'draft' ? (
          <div hidden={!editing}>
        ...
        <div hidden={editing} className="space-y-5">
          <p>{draft.reason}</p>
          <Alert variant="info">
            <AlertDescription>{t('unpublished')}</AlertDescription>
          </Alert>
          <div className="flex gap-2">
            <Button asChild variant="outline">
              <Link
                href={`/admin/apps/preview/${draft.id}` as never}
                target="_blank"
              >
                {t('preview')}
              </Link>
          ...
          <section className="space-y-2">
            ...
            {(['added', 'changed', 'removed'] as const).map((kind) =>
              draft.changes[kind].length ? (
                <p key={kind} className="break-all text-sm">
                  <strong>{t(kind)}: </strong>
                  {draft.changes[kind].join(', ')}
                </p>
              ) : null,
            )}
          ...
          <section>
            <h3 className="font-semibold">{t('contents')}</h3>
          ...
          {(manifest.contributions ?? [])
            .filter((item) => item.kind === 'page' && !item.route.includes('['))
            .map((item) =>
              item.kind === 'page' ? (
                <Button
                  key={item.route}
