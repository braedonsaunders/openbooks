...
export function AppPackageEditor({
  ...
  const [pending, setPending] = useState({ screens: false, definitions: false })
  ...
  async function uploadFiles(list: FileList | null, directory = false) {
    ...
    try {
      ...
      for (const file of Array.from(list)) {
        ...
        if (content.length > 2 * 1024 * 1024) throw new Error(t('fileTooLarge'))
  ...
  return (
    <div className="min-w-0 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800">
        <div className="flex" role="tablist" aria-label={t('tabsLabel')}>
      ...
      {tab === 'overview' ? (
        manifest ? (
          <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onChange={(e) =>
                    patchManifest({
                      frontend: { ...manifest.frontend, entry: e.target.value },
                    })
                  }
                >
                  {files
                    .filter(
                      (file) => file.path !== 'manifest.json' && !file.isBinary,
                    )
                    .map((file) => (
            ...
            <AppDefinitions
              onPendingChange={(value) => pendingChanged('definitions', value)}
              files={files}
              manifest={manifest}
              onFiles={change}
