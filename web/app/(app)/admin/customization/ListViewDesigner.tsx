...
export function ListViewDesigner({
  ...
  const meta = catalog
    ? recordTypeForFeatureState(catalog, { inventory: inventoryEnabled, crm: crmEnabled, hrm: hrmEnabled })
    : catalog
          ...
          <div className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
            <div className="space-y-1.5">
              {view.columns.map((c, ci) => {
                const locked = isLockedCustomizationEntry(meta?.listColumns, c.key)
                return (
                <div key={c.key} className="flex flex-wrap items-center gap-2 rounded-md border border-slate-100 px-2.5 py-1.5 dark:border-slate-800">
                  ...
                  {locked ? <span className="rounded border px-1 text-[10px] text-slate-500">{t('designer.forms.locked')}</span> : null}
                  <Input value={c.labelOverride ?? ''} onChange={(e) => updateCol(ci, { labelOverride: e.target.value || null })} placeholder={colLabel(c.key)} className="h-7 w-36" disabled={locked} />
                  <div className="ml-auto flex items-center gap-1">
                    <button type="button" onClick={() => setView((p) => { const n = structuredClone(p) as ListViewConfig; n.columns = reorder(n.columns, ci, ci - 1); return n })} disabled={locked} aria-label={`${tCommon('actions.previous')}: ${colLabel(c.key)}`} className="text-slate-400 hover:text-slate-600 disabled:cursor-not-allowed disabled:opacity-40"><ChevronUp size={15} /></button>
                    <button type="button" onClick={() => setView((p) => { const n = structuredClone(p) as ListViewConfig; n.columns = reorder(n.columns, ci, ci + 1); return n })} disabled={locked} aria-label={`${tCommon('actions.next')}: ${colLabel(c.key)}`} className="text-slate-400 hover:text-slate-600 disabled:cursor-not-allowed disabled:opacity-40"><ChevronDown size={15} /></button>
                    <button type="button" onClick={() => updateCol(ci, { visible: !c.visible })} disabled={locked} className={cn('text-slate-400 hover:text-slate-600 disabled:cursor-not-allowed disabled:opacity-40', !c.visible && 'text-red-500')} aria-label={t('designer.list.visible')}>
