...
const SORTS = { subject: sql`a.subject`, due: sql`a.due_at`, type: sql`a.kind`, status: sql`a.status`, owner: sql`u.name` } as const
export default async function Activities({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  const authz = await requirePermission('crm.activities.read'); const manage = can(authz, 'crm.activities.manage'); const t = await getTranslations('crm'); const sp = await searchParams
  const list = parseListParams(sp, { sort: 'due', dir: 'asc', perPage: 25, allowedSorts: ['subject','due','type','status','owner'] as const }); const kind=pickString(sp.kind); const status=pickString(sp.status); const owner=pickString(sp.owner); const openId=pickString(sp.activity)
  const where=sql`a.org_id=${authz.user.orgId} ${list.q?sql`and (a.subject ilike ${`%${list.q}%`} or a.body ilike ${`%${list.q}%`})`:sql``} ${kind?sql`and a.kind=${kind}`:sql``} ${status?sql`and a.status=${status}`:sql``} ${owner&&isUuid(owner)?sql`and a.assigned_user_id=${owner}`:sql``}`
  const [rows,count,owners,accounts,opportunities,open]=await Promise.all([
    db.execute(sql`select a.*,u.name assigned_name from crm_activities a left join users u on u.id=a.assigned_user_id where ${where} order by ${SORTS[list.sort]} ${list.dir==='asc'?sql`asc`:sql`desc`} nulls last limit ${list.perPage} offset ${(list.page-1)*list.perPage}`) as any,
    db.execute(sql`select count(*)::int n from crm_activities a left join users u on u.id=a.assigned_user_id where ${where}`) as any,
  ...
  return <ListPageLayout header={<><PageHeader title={t('activities.title')} description={t('activities.description')} actions={button}/><div className="flex flex-wrap gap-2"><SearchInput placeholder={t('activities.search')}/><FilterChips basePath="/crm/activities" currentParams={sp} paramKey="kind" label={t('fields.activityType')} options={['task','call','event','email','note'].map(v=>({value:v,label:t(`activityKinds.${v}`)}))}/><FilterChips basePath="/crm/activities" currentParams={sp} paramKey="status" label={t('fields.status')} options={['planned','in_progress','completed','cancelled'].map(v=>({value:v,label:t(`activityStatuses.${v}`)}))}/><FilterChips basePath="/crm/activities" currentParams={sp} paramKey="owner" label={t('fields.assignedTo')} options={owners.rows.map((o:any)=>({value:o.id,label:o.name}))}/></div></>}>
    {!rows.rows.length?<EmptyState title={t('activities.emptyTitle')} description={t('activities.emptyDescription')} action={button}/>:<><Table><TableHeader><TableRow><SortTh basePath="/crm/activities" currentParams={sp} column="subject" sort={list.sort} dir={list.dir}>{t('fields.subject')}</SortTh><SortTh basePath="/crm/activities" currentParams={sp} column="type" sort={list.sort} dir={list.dir}>{t('fields.activityType')}</SortTh><SortTh basePath="/crm/activities" currentParams={sp} column="status" sort={list.sort} dir={list.dir}>{t('fields.status')}</SortTh><SortTh basePath="/crm/activities" currentParams={sp} column="owner" sort={list.sort} dir={list.dir}>{t('fields.assignedTo')}</SortTh><SortTh basePath="/crm/activities" currentParams={sp} column="due" sort={list.sort} dir={list.dir}>{t('fields.due')}</SortTh></TableRow></TableHeader><TableBody>{rows.rows.map((r:any)=><TableRow key={r.id}><TableCell><Link href={`/crm/activities?activity=${r.id}`} className="font-semibold text-teal-700 hover:underline dark:text-teal-300">{r.subject}</Link></TableCell><TableCell>{t(`activityKinds.${r.kind}`)}</TableCell><TableCell><Badge variant={r.status==='completed'?'success':'outline'}>{t(`activityStatuses.${r.status}`)}</Badge></TableCell><TableCell>{r.assigned_name??t('fields.unassigned')}</TableCell><TableCell className={r.due_at&&new Date(r.due_at)<new Date()&&r.status!=='completed'?'text-red-600':''}>{r.due_at?new Date(r.due_at).toLocaleString():'—'}</TableCell></TableRow>)}</TableBody></Table><Pagination basePath="/crm/activities" currentParams={sp} total={Number(count.rows[0]?.n??0)} page={list.page} perPage={list.perPage}/></>}
