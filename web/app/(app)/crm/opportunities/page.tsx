...
export default async function Opportunities({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 const [rows,count,statuses,owners,accounts,contacts,teams,sources,items,open]=await Promise.all([
 ...
 {open?<OpportunityDrawer data={open} statuses={statuses.rows} owners={owners.rows} accounts={accounts.rows} contacts={contacts.rows} teams={teams.rows} sources={sources.rows} items={items.rows} canManage={manage}/>:null}
 </ListPageLayout>
}
