...
import { loadSavedSearches, type SavedSearchRow } from '../../../../lib/saved-searches'
import { NewSearchButton } from './NewSearchButton'
import { SavedSearchStudio } from './SavedSearchStudio'
...
export default async function SavedSearchesPage({
  ...
  const all = await loadSavedSearches(authz.user.orgId, authz.user.id, authz.permissions)
  ...
  const openId = pickString(sp.search)
  const openSearch = openId ? all.find((s) => s.id === openId) ?? null : null
  ...
  return (
      ...
      header={
        <>
          <PageHeader
            ...
            actions={canCreate ? <NewSearchButton /> : null}
          />
          <div className="flex flex-wrap items-center gap-2">
            <SearchInput placeholder={t('list.searchPlaceholder')} />
            <FilterChips
              basePath="/knowledge/saved-searches"
    ...
    >
          ...
          <Table>
            ...
            <TableBody>
              {rows.map((s) => (
                <TableRow key={s.id}>
                  <TableCell>
                    {/* Name RUNS the search (NetSuite behaviour); editing is the
                        explicit action on the right. */}
                    <Link href={`/knowledge/saved-searches/${s.id}`} className="font-medium text-teal-700 hover:underline dark:text-teal-300">
                      {s.name}
                    </Link>
                    {s.description ? (
                  ...
                  <TableCell>
                    <div className="flex items-center gap-3 text-sm">
                      <Link href={`/knowledge/saved-searches/${s.id}`} className="text-teal-700 hover:underline dark:text-teal-300">
                        {t('list.runAction')}
                      </Link>
                      {authz.permissions.has('*') || s.owner_id === authz.user.id ? (
                        <Link href={`/knowledge/saved-searches?search=${s.id}`} className="text-slate-500 hover:text-slate-900 hover:underline dark:text-slate-400 dark:hover:text-slate-100">
                          {t('list.editAction')}
                        </Link>
                      ) : null}
                    </div>
      ...
      <Pagination basePath="/knowledge/saved-searches" currentParams={currentParams} page={params.page} perPage={PER_PAGE} total={total} />

      {openSearch ? <SavedSearchStudio search={openSearch} canCreate={canCreate} canAdmin={authz.permissions.has('*') || openSearch.owner_id === authz.user.id} /> : null}
    </ListPageLayout>
  )
}
