...
export default async function SavedSearchRunPage({
  params,
  searchParams,
}: {
  ...
  const t = await getTranslations('knowledge.savedSearches')
  ...
  const search = /^[0-9a-f-]{36}$/i.test(id)
    ? await loadSavedSearch(authz.user.orgId, id, authz.user.id, authz.permissions)
    : null
      ...
      <ListPageLayout
        header={
          <PageHeader
            ...
            back={{ href: '/knowledge/saved-searches', label: t('run.backToList') }}
          />
        }
      >
  ...
  const canEdit = authz.permissions.has('*') || search.owner_id === authz.user.id
  const result = await runSavedSearch(authz.user.orgId, search.query)
  ...
  const entityLabel = tReports.has(`catalog.entities.${search.query.entity}.label`)
    ? tReports(`catalog.entities.${search.query.entity}.label`)
    : search.query.entity
  ...
  return (
      ...
      header={
        <>
          <DetailHeader
            title={search.name}
            badge={
              <Badge variant={search.scope === 'shared' ? 'secondary' : 'outline'}>
                {t(`list.scope.${search.scope}` as never)}
              </Badge>
            }
            subtitle={search.description || t('run.refreshHint')}
            back={{ href: '/knowledge/saved-searches', label: t('run.backToList') }}
            actions={
              <div className="flex flex-wrap items-center gap-2">
                <Button variant="outline" size="sm" asChild>
                  <a href={`/api/saved-searches/${search.id}/export?format=pdf`}><FileText size={15} /> {t('studio.exportPdf')}</a>
                </Button>
                <Button variant="outline" size="sm" asChild>
                  <a href={`/api/saved-searches/${search.id}/export?format=xlsx`}><Download size={15} /> {t('studio.exportXlsx')}</a>
                </Button>
                <Button variant="outline" size="sm" asChild>
                  <a href={`/api/saved-searches/${search.id}/export?format=csv`}><Download size={15} /> {t('studio.exportCsv')}</a>
                </Button>
                {canEdit ? (
                  <Button size="sm" asChild>
                    <Link href={`/knowledge/saved-searches?search=${search.id}`}><Pencil size={14} /> {t('run.editSearch')}</Link>
                  </Button>
                ) : null}
              </div>
            }
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
            ...
            <span>{t('run.lastUpdated', { when: dateTime(search.updated_at) })}</span>
    ...
    >
      ...
      <Pagination
        basePath={`/knowledge/saved-searches/${search.id}`}
