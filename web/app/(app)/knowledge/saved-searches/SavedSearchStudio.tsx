...
/**
 * The saved-search builder flyout. Builds the SAME ReportCustomQuery plan as
 * the report studio (shared RowsConfig/SummarizeConfig/FilterTree/ResultView),
 * adds saved-search metadata (name, scope), autosaves, runs a live preview via
 * the shared report run route, and exports to PDF/Excel/CSV.
 */
export function SavedSearchStudio({
  search,
  canCreate,
  canAdmin,
}: {
  ...
  const t = useTranslations('knowledge.savedSearches.studio')
  ...
  const [query, setQuery] = useState<ReportCustomQuery>(search.query)
  const [scope, setScope] = useState<SavedSearchScope>(search.scope)
  ...
  async function remove() {
    ...
    if (res.ok) {
      router.push('/knowledge/saved-searches')
      router.refresh()
    } else {
  ...
  return (
    <UrlDrawer
      open
      closeHref="/knowledge/saved-searches"
      size="2xl"
      title={
      ...
      headerActions={
        <>
          <Button variant="outline" asChild>
            <Link href={`/knowledge/saved-searches/${search.id}`}><ExternalLink size={15} /> {t('viewResults')}</Link>
          </Button>
          <Button variant="outline" asChild>
            <a href={`/api/saved-searches/${search.id}/export?format=pdf`}><FileText size={15} /> {t('exportPdf')}</a>
          </Button>
          <Button variant="outline" asChild>
            <a href={`/api/saved-searches/${search.id}/export?format=xlsx`}><Download size={15} /> {t('exportXlsx')}</a>
          </Button>
          <Button variant="outline" asChild>
            <a href={`/api/saved-searches/${search.id}/export?format=csv`}><Download size={15} /> {t('exportCsv')}</a>
          </Button>
          {canAdmin ? (
        ...
        <div className={field}>
          ...
          {canCreate ? (
            <div className="flex items-center gap-3">
              ...
              <Select className="h-8 w-40" value={scope} onChange={(e) => setScope(e.target.value as SavedSearchScope)} disabled={!canCreate}>
