 ...
 * that names it. Shared by the payroll routes: profiles, pay-run adjustments
 * and anywhere else an operator-supplied decimal crosses the wire.
 *
 * Moved verbatim out of `web/app/api/payroll/profiles/route.ts` so every
 * site classifies the same way. A second implementation of this classifier
 * would be a defect even if every message read well: the decimal-comma
 * reading is a 100x money error when it is wrong, and this logic already
 * cost three rounds to get right. Fix it here once, never beside a route.
 */
