...
test('analytics copy ships translated in fr, es and de', () => {
  ...
  assert.equal(wanted.length, 1911, 'analytics source inventory changed; translate the new keys everywhere and re-pin')
  for (const key of wanted) {
    const english = source.get(key)
    assert.ok(english && english.trim(), `English source is missing ${key}`)
  }
...
test('admin namespace ships translated in zh and pt-BR', () => {
  ...
  assert.equal(wanted.length, 4062, 'admin source inventory changed; translate the new keys in zh/pt-BR and re-pin')
  for (const key of wanted) {
    const english = source.get(key)
    assert.ok(english && english.trim(), `English source is missing ${key}`)
  }
...
test('admin copy ships translated in de and ja (i2)', () => {
  ...
  const ADMIN_I2_SOURCE_COUNT = 4062
  const ADMIN_I2_SOURCE_HASH = 'cf870992d82fb42cbd92636dfddf9597e8d7d2cafa9072b674ab5a849edc93eb'
  const source = flattenCatalog('en')
  const sourceKeys = [...source.keys()]
