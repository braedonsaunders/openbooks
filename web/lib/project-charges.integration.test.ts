      ...
      try {
        ...
        await db.execute(sql\`
          insert into projects
            (id, org_id, subsidiary_id, code, name, customer_id, status, is_active, custom)
          values
            (\${projectId}, \${org.orgId}, \${org.subsidiaryId}, 'CHARGE-ACCOUNT',
             'Charge account job', \${org.customerId}, 'active', true, '{}'::jsonb)
        \`);
        await db.execute(sql\`
          insert into items
            (id, org_id, kind, code, name, default_cost, default_rate,
             ...
             is_active, custom)
          values
            (\${itemId}, \${org.orgId}, 'service', 'CHG-ACCOUNT', 'Charge account service',
             '10.0000', '15.0000', \${org.accounts.cogs}, \${org.accounts.adjustment},
             \${org.accounts.revenue}, true, '{}'::jsonb)
        \`);
