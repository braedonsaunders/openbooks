...
test('custom source publication, audience, app grants and schema drift are enforced at execution', {skip:!process.env.OPENBOOKS_DB_URL}, async()=>{
  ...
  const orgId=org.orgId,userId=randomUUID(),typeId=randomUUID(),otherOrg=other.orgId
  ...
  try {
    ...
    await withBypassContext(() => (db.execute(sql`insert into custom_record_types(id,org_id,key,name,plural_name,fields,status) values(${typeId},${orgId},'query-fixture','Query fixture','Query fixtures',${JSON.stringify(fields)}::jsonb,'published')`)))
    for(const [org,n,number] of [[orgId,'10.25','QA-1'],[otherOrg,'999','QA-OTHER']] as const) await withBypassContext(() => (db.execute(sql`insert into custom_records(id,org_id,type_id,type_key,record_number,status,data) values(${randomUUID()},${org},${typeId},'query-fixture',${number},'active',${JSON.stringify({hours:n,related:number})}::jsonb)`)))
    await withBypassContext(() => (db.execute(sql`insert into custom_records(id,org_id,type_id,type_key,record_number,status,data) values(${randomUUID()},${orgId},${typeId},'query-fixture','QA-2','active','{"hours":"2","related":"QA-2"}'::jsonb)`)))
    ...
    await withBypassContext(() => (db.execute(sql`update custom_record_types set allowed_roles='["private-role"]'::jsonb where id=${typeId}`)))
    ...
    await withBypassContext(() => (db.execute(sql`update custom_record_types set allowed_roles=null,status='archived' where id=${typeId}`)))
    assert.equal((await reportEntityCatalog(authz))['custom:query-fixture'],undefined)
    await assert.rejects(()=>platform.query!(plan),/unavailable/)
  } finally {
