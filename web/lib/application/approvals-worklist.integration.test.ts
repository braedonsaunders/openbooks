...
test("worklist unifies gates, gateless documents, and pay runs; vitals counts the same set", { skip: !DB }, async () => {
  ...
  try {
    ...
    const ids = await seed(org.orgId, org.subsidiaryId, org.accounts.bank, actors);
    const approver = ctxFor(org.orgId, actors.approver1Id, ["approver"], ["flows.approve", "ap.approve"]);
    const items = await listApprovalWorklist(approver);
    ...
    const vitals = await orgVitals(approver);
    ...
    const adminItems = await listApprovalWorklist(admin);
...
test("decide paths resolve each subject kind with separation of duties", { skip: !DB }, async () => {
  ...
  try {
    ...
    assert.equal(await docStatus(ids.gatelessId), "approved");
    ...
    const run = await decideApproval(approver, {
    ...
    const runStatus = (await db.execute<{ status: string }>(sql`select status from payment_runs where id = ${ids.runId}`)).rows[0]?.status;
    ...
    await assert.rejects(
      decideApproval(submitter, { paymentRunId: runId2, decision: "approved", idempotencyKey: randomUUID() }),
      /submitter cannot approve/,
