...
test("a duplicate waits out an in-flight attempt without pinning a pool client, then replays", async () => {
...
test("a rolled-back leader hands the command to the polling duplicate", async () => {
  const h = await buildIdempotencyHarness();
  try {
    const gate = deferred();
    const leader = h.executeIdempotent({
...
test("key reuse with different payload conflicts even against an in-flight rival", async () => {
  const h = await buildIdempotencyHarness();
  try {
    const gate = deferred();
    const leader = h.executeIdempotent({
...
test("a duplicate gives up with a conflict once the rival wait budget lapses", async () => {
  const h = await buildIdempotencyHarness();
  try {
    const gate = deferred();
    const leader = h.executeIdempotent({
...
test("a duplicate-key void storm stays exact-once and leaves the request pool responsive", async () => {
