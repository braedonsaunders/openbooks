...
async function oppRev(orgId: string, id: string) {
 return (await withOrgContext(orgId, () => db.execute<{revision:string}>(sql`select (revision_seq)::text as revision from crm_opportunities where org_id=${orgId} and id=${id}`))).rows[0]!.revision;
}
...
async function fixture(action: (org: Fixture, open: string, closed: string) => Promise<void>) {
 ...
 try {
  const seeded=await withBypassContext(async () => {
   ...
   return {actor,open:statuses.find(s=>!s.is_closed)!.id,closed:statuses.find(s=>s.is_closed)!.id};
  });
  session.user={id:seeded.actor,orgId:org.orgId,name:'Reviewer',email:'reviewer@example.test',roles:[],isSuperAdmin:false,envKind:'production',productionOrgId:org.orgId,homeOrgId:org.orgId,homeUserId:seeded.actor};
  await action(org,seeded.open,seeded.closed);
 } finally {session.user=null;await dropScratchOrg(org.orgId);}
}
async function opportunity(org: Fixture, status: string, active=true) {
 const id=randomUUID();
 await withBypassContext(() => db.execute(sql`insert into crm_opportunities(id,org_id,opportunity_number,title,party_id,status_id,currency,is_active)
...
test('party retirement refuses open opportunities without changing state or audit history', enabled, async()=>fixture(async(org,open,closed)=>{
 ...
 const before=(await withOrgContext(org.orgId,()=>db.execute<{n:string}>(sql`select count(*)::text as n from audit_log where org_id=${org.orgId}`))).rows[0]!.n;
 ...
 assert.equal((await withOrgContext(org.orgId,()=>db.execute<{is_active:boolean}>(sql`select is_active from parties where id=${org.customerId}`))).rows[0]!.is_active,true);
 assert.equal((await withOrgContext(org.orgId,()=>db.execute<{n:string}>(sql`select count(*)::text as n from audit_log where org_id=${org.orgId}`))).rows[0]!.n,before);
 await withBypassContext(()=>db.execute(sql`update crm_opportunities set status_id=${closed} where id=${id}`));
 const accepted=await withOrgContext(org.orgId,async()=>PATCH(await retirement(org),params(org.customerId)));
 assert.equal(accepted.status,200,await accepted.clone().text());
 assert.equal((await withOrgContext(org.orgId,()=>db.execute<{is_active:boolean}>(sql`select is_active from parties where id=${org.customerId}`))).rows[0]!.is_active,false);
}));
...
test('inactive accounts refuse new and reopened opportunities but allow closing legacy work', enabled, async()=>fixture(async(org,open,closed)=>{
 const id=await opportunity(org,open);
 await withBypassContext(()=>db.execute(sql`update parties set is_active=false where id=${org.customerId}`));
 ...
 assert.equal((await withOrgContext(org.orgId,()=>db.execute<{status_id:string}>(sql`select status_id from crm_opportunities where id=${id}`))).rows[0]!.status_id,closed);
}));
...
test('opportunity activation rechecks account retirement after waiting for its lock', enabled, async()=>fixture(async(org,open)=>{
 ...
 try {
  ...
  const response=await pending;
  assert.equal(response.status,422,await response.clone().text());
  assert.equal((await withOrgContext(org.orgId,()=>db.execute<{is_active:boolean}>(sql`select is_active from crm_opportunities where id=${id}`))).rows[0]!.is_active,false);
 } finally {await client.query('rollback');client.release();await pending?.catch(()=>{});}
}));
