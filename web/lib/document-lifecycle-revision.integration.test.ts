...
test(`interactive document lifecycle: ${operation}`, { skip: !process.env.OPENBOOKS_DB_URL }, async()=>{
 ...
 try{
  ...
  const {actor,id,token}=await withBypassContext(async()=>{
   const seededActor=await createScratchUser(org.orgId,'Document controller','reviewer');
   ...
   await db.execute(sql`insert into documents(id,org_id,kind,document_number,document_date,party_id,subsidiary_id,currency) values (${seededId},${org.orgId},'customer_invoice',${seededId},${org.date},${org.customerId},${org.subsidiaryId},'CAD')`);
   await db.execute(sql`insert into document_lines(org_id,document_id,line_number,account_id,quantity,unit_price,amount) values (${org.orgId},${seededId},1,${org.accounts.revenue},1,'100','100')`);
   if (operation.startsWith('void')) await db.execute(sql`update documents set status='approved' where id=${seededId}`);
   const seededToken=(await db.execute<{revision:string}>(sql`select (revision_seq)::text as revision from documents where id=${seededId}`)).rows[0]!.revision;
   if (operation.endsWith('stale')) await db.execute(sql`update documents set memo='Concurrent change',updated_at=updated_at+interval '1 microsecond' where id=${seededId}`);
   return {actor:seededActor,id:seededId,token:seededToken};
  });
