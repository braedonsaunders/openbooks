...
test('account option reader includes only accounts assigned to the caller subsidiaries', { skip: !process.env.OPENBOOKS_DB_URL }, async () => {
  await withBypassContext(async () => {
    ...
    try {
      ...
      assert.ok(!restricted.some((account) => account.id === hiddenAccount))
      assert.ok(restricted.some((account) => account.id === sharedAccount), 'null subsidiary is shared master data')
