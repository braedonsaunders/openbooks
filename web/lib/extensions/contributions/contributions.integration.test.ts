...
import { parseManifest } from '../../apps/manifest'


// Persistence, signature, withdrawal and effective-date evidence are exercised
// against PostgreSQL in engine/src/extensions/projections.integration.test.ts.
test('web contribution parsers and engine persistence share exactly the same payload validation', () => {
  ...
  const parsed = parseManifest({...manifest,frontend:{entry:'frontend/ui.json',renderer:'native'},endpoints:[]})
