...
const RATE_VERSION_STATUS = [
  { value: 'draft', labelKey: 'options.rateVersionStatus.draft' },
  { value: 'active', labelKey: 'options.rateVersionStatus.active' },
  { value: 'retired', labelKey: 'options.rateVersionStatus.retired' },
]

const COMPENSATION_BASIS = [
  { value: 'hour', labelKey: 'options.compensationBasis.hour' },
  { value: 'year', labelKey: 'options.compensationBasis.year' },
]

const LABOR_RATE_LANES = [
  { value: 'direct_cost', labelKey: 'options.laborRateLane.directCost' },
  { value: 'bill', labelKey: 'options.laborRateLane.bill' },
  { value: 'transfer', labelKey: 'options.laborRateLane.transfer' },
  { value: 'planning_cost', labelKey: 'options.laborRateLane.planningCost' },
  { value: 'planning_bill', labelKey: 'options.laborRateLane.planningBill' },
]

const LABOR_RATE_METHODS = [
  { value: 'fixed', labelKey: 'options.laborRateMethod.fixed' },
  { value: 'at_cost', labelKey: 'options.laborRateMethod.atCost' },
  { value: 'markup_on_cost', labelKey: 'options.laborRateMethod.markupOnCost' },
  { value: 'margin_on_cost', labelKey: 'options.laborRateMethod.marginOnCost' },
]

const LABOR_COMPONENT_LANES = [
  { value: 'cost', labelKey: 'options.laborComponentLane.cost' },
  { value: 'bill', labelKey: 'options.laborComponentLane.bill' },
  { value: 'transfer', labelKey: 'options.laborComponentLane.transfer' },
]

const LABOR_COMPONENT_METHODS = [
  { value: 'fixed_per_hour', labelKey: 'options.laborComponentMethod.fixedPerHour' },
  { value: 'percent_of_base_direct', labelKey: 'options.laborComponentMethod.percentOfBaseDirect' },
  { value: 'percent_of_direct', labelKey: 'options.laborComponentMethod.percentOfDirect' },
  { value: 'percent_of_subtotal', labelKey: 'options.laborComponentMethod.percentOfSubtotal' },
]

const PAYROLL_COMPONENTS = [
  { value: 'gross_pay', labelKey: 'options.payrollComponent.grossPay' },
  { value: 'employer_tax', labelKey: 'options.payrollComponent.employerTax' },
  { value: 'benefit', labelKey: 'options.payrollComponent.benefit' },
  { value: 'worker_comp', labelKey: 'options.payrollComponent.workerComp' },
  { value: 'other', labelKey: 'options.payrollComponent.other' },
]

const EXTERNAL_PAYROLL_ACCOUNTING_MODES = [
  { value: 'costing_only', labelKey: 'options.externalPayrollAccountingMode.costingOnly' },
  { value: 'variance_to_clearing', labelKey: 'options.externalPayrollAccountingMode.varianceToClearing' },
]
...
export const SETUP_ENTITIES: SetupEntity[] = [
  ...
  },
  {
    ...
    columns: [
      ...
      { key: 'projectTaskId', kind: 'ref', ref: 'project-tasks' },
      { key: 'subsidiaryId', kind: 'ref', ref: 'subsidiaries' },
    ...
    fields: [
      ...
      { key: 'priority', kind: 'integer', keepDefault: true },
      { key: 'effectiveFrom', kind: 'date' },
      { key: 'effectiveTo', kind: 'date' },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  },
  {
    ...
    fields: [
      { key: 'rateBookId', kind: 'ref', ref: 'item-rate-books', required: true, lockedOnEdit: true },
      { key: 'effectiveFrom', kind: 'date', required: true, lockedOnEdit: true },
      { key: 'effectiveTo', kind: 'date' },
      { key: 'status', kind: 'select', options: RATE_VERSION_STATUS, keepDefault: true, defaultValue: 'draft' },
    ],
  },
  ...
  // --- Workforce -----------------------------------------------------------
  {
    ...
    columns: [
      { key: 'code', kind: 'code' },
      { key: 'name', kind: 'text' },
      { key: 'parentId', kind: 'ref', ref: 'labor-classes' },
      { key: 'isActive', kind: 'badge-active' },
    ],
    fields: [
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
  ...
  {
    key: 'employee-labor-class-assignments',
    table: 'employee_labor_class_assignments',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'users',
    orgScoped: true,
    ...
    columns: [
      ...
      { key: 'laborClassId', kind: 'ref', ref: 'labor-classes' },
      { key: 'effectiveFrom', kind: 'date' },
      { key: 'effectiveTo', kind: 'date' },
      { key: 'isPrimary', kind: 'boolean' },
      { key: 'isActive', kind: 'badge-active' },
    ...
    fields: [
      { key: 'employeePartyId', kind: 'ref', ref: 'employees', required: true },
      { key: 'laborClassId', kind: 'ref', ref: 'labor-classes', required: true },
      { key: 'effectiveFrom', kind: 'date', required: true },
      { key: 'effectiveTo', kind: 'date' },
      { key: 'isPrimary', kind: 'boolean', defaultValue: true },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  },
  {
    key: 'employee-compensation-rates',
    table: 'employee_compensation_rates',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'payments',
    orgScoped: true,
    ...
    fields: [
      ...
      { key: 'currency', kind: 'ref', ref: 'currencies', required: true },
      { key: 'basis', kind: 'select', options: COMPENSATION_BASIS, keepDefault: true, defaultValue: 'hour' },
      { key: 'annualHours', kind: 'decimal', keepDefault: true, defaultValue: 2080 },
      { key: 'effectiveFrom', kind: 'date', required: true },
      { key: 'effectiveTo', kind: 'date' },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  ...
  {
    key: 'labor-rate-lines',
    table: 'labor_rate_lines',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'tag',
    orgScoped: true,
    orderBy: 'version_id, lane, priority desc, code',
    hasActive: true,
    docSlug: 'labor-rates',
    columns: [
      { key: 'code', kind: 'code' },
      { key: 'name', kind: 'text' },
      { key: 'versionId', kind: 'ref', ref: 'item-rate-versions' },
      { key: 'lane', kind: 'text' },
      { key: 'method', kind: 'text' },
      { key: 'amount', kind: 'number' },
    ...
    fields: [
      { key: 'versionId', kind: 'ref', ref: 'item-rate-versions', required: true, lockedOnEdit: true },
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'name', kind: 'text', required: true },
      { key: 'lane', kind: 'select', options: LABOR_RATE_LANES, required: true },
      { key: 'method', kind: 'select', options: LABOR_RATE_METHODS, required: true },
      { key: 'amount', kind: 'decimal' },
      { key: 'percent', kind: 'percent' },
      { key: 'currency', kind: 'ref', ref: 'currencies', required: true },
      { key: 'baseHours', kind: 'decimal', keepDefault: true, defaultValue: 1 },
      { key: 'employeePartyId', kind: 'ref', ref: 'employees' },
      { key: 'laborClassId', kind: 'ref', ref: 'labor-classes' },
      { key: 'itemId', kind: 'ref', ref: 'items' },
      ...
      { key: 'departmentId', kind: 'ref', ref: 'departments' },
      { key: 'locationId', kind: 'ref', ref: 'locations' },
      { key: 'workerCompGroupId', kind: 'ref', ref: 'worker-comp-groups' },
      { key: 'priority', kind: 'integer', keepDefault: true },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  },
  {
    key: 'labor-rate-components',
    table: 'labor_rate_components',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'percent',
    orgScoped: true,
    orderBy: 'version_id, lane, sequence, code',
    hasActive: true,
    docSlug: 'labor-rates',
    columns: [
      { key: 'code', kind: 'code' },
    ...
    fields: [
      ...
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'name', kind: 'text', required: true },
      { key: 'lane', kind: 'select', options: LABOR_COMPONENT_LANES, required: true },
      { key: 'method', kind: 'select', options: LABOR_COMPONENT_METHODS, required: true },
      { key: 'value', kind: 'decimal', required: true },
      { key: 'currency', kind: 'ref', ref: 'currencies' },
      { key: 'employeePartyId', kind: 'ref', ref: 'employees' },
      { key: 'laborClassId', kind: 'ref', ref: 'labor-classes' },
      { key: 'itemId', kind: 'ref', ref: 'items' },
      { key: 'timeTypeId', kind: 'ref', ref: 'time-types' },
  ...
  {
    key: 'external-payroll-sources',
    table: 'external_payroll_sources',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'plug',
    orgScoped: true,
    ...
    columns: [
      { key: 'code', kind: 'code' },
      { key: 'name', kind: 'text' },
      { key: 'accountingMode', kind: 'text' },
      { key: 'payrollClearingAccountId', kind: 'ref', ref: 'accounts' },
      { key: 'requirePostedJournal', kind: 'boolean' },
      { key: 'isActive', kind: 'badge-active' },
    ],
    fields: [
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'name', kind: 'text', required: true },
      { key: 'accountingMode', kind: 'select', options: EXTERNAL_PAYROLL_ACCOUNTING_MODES, keepDefault: true, defaultValue: 'variance_to_clearing' },
      { key: 'payrollClearingAccountId', kind: 'ref', ref: 'accounts' },
      { key: 'requirePostedJournal', kind: 'boolean', defaultValue: true },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  },
  {
    key: 'external-payroll-import-templates',
    table: 'external_payroll_import_templates',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'file-spreadsheet',
    orgScoped: true,
    orderBy: 'source_id, code',
    hasActive: true,
    docSlug: 'labor-rates',
    columns: [
      { key: 'code', kind: 'code' },
      { key: 'name', kind: 'text' },
      { key: 'sourceId', kind: 'ref', ref: 'external-payroll-sources' },
      { key: 'employeeCodeColumn', kind: 'text' },
      { key: 'amountColumn', kind: 'text' },
      { key: 'isActive', kind: 'badge-active' },
    ],
    fields: [
      { key: 'sourceId', kind: 'ref', ref: 'external-payroll-sources', required: true, lockedOnEdit: true },
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'name', kind: 'text', required: true },
      { key: 'externalLineIdColumn', kind: 'text', required: true, keepDefault: true, defaultValue: 'externalLineId' },
      { key: 'employeeCodeColumn', kind: 'text', required: true, keepDefault: true, defaultValue: 'employeePartyId' },
      { key: 'componentColumn', kind: 'text', required: true, keepDefault: true, defaultValue: 'component' },
      { key: 'amountColumn', kind: 'text', required: true, keepDefault: true, defaultValue: 'amount' },
      { key: 'payCodeColumn', kind: 'text' },
      { key: 'hoursColumn', kind: 'text' },
      { key: 'memoColumn', kind: 'text' },
      { key: 'isActive', kind: 'boolean', defaultValue: true },
    ],
  },
  {
    key: 'payroll-cost-batches',
    table: 'payroll_cost_batches',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'payments',
    orgScoped: true,
    naturalKey: 'code',
    orderBy: 'period_end desc, code',
    hasActive: false,
    docSlug: 'labor-rates',
    columns: [
      { key: 'code', kind: 'code' },
      { key: 'sourceId', kind: 'ref', ref: 'external-payroll-sources' },
      { key: 'externalBatchId', kind: 'text' },
      { key: 'subsidiaryId', kind: 'ref', ref: 'subsidiaries' },
      { key: 'periodStart', kind: 'date' },
      { key: 'periodEnd', kind: 'date' },
      { key: 'postingDate', kind: 'date' },
      { key: 'actualTotal', kind: 'number' },
      { key: 'varianceTotal', kind: 'number' },
      { key: 'status', kind: 'text' },
    ],
    fields: [
      { key: 'code', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'sourceId', kind: 'ref', ref: 'external-payroll-sources', required: true, lockedOnEdit: true },
      { key: 'externalBatchId', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'subsidiaryId', kind: 'ref', ref: 'subsidiaries', required: true, lockedOnEdit: true },
      { key: 'periodStart', kind: 'date', required: true },
      { key: 'periodEnd', kind: 'date', required: true },
      { key: 'postingDate', kind: 'date', required: true },
      { key: 'currency', kind: 'ref', ref: 'currencies', required: true },
      { key: 'sourceJournalDocumentId', kind: 'ref', ref: 'posted-payroll-journals' },
    ],
  },
  {
    key: 'payroll-cost-lines',
    table: 'payroll_cost_lines',
    actorCols: true,
    groupKey: 'workforce',
    iconKey: 'payments',
    orgScoped: true,
    orderBy: 'batch_id, employee_party_id, component',
    hasActive: false,
    docSlug: 'labor-rates',
    columns: [
      { key: 'batchId', kind: 'ref', ref: 'payroll-cost-batches' },
      { key: 'externalLineId', kind: 'code' },
      { key: 'employeePartyId', kind: 'ref', ref: 'employees' },
      { key: 'payCode', kind: 'text' },
      { key: 'component', kind: 'text' },
      { key: 'amount', kind: 'number' },
      { key: 'memo', kind: 'text' },
    ],
    fields: [
      { key: 'batchId', kind: 'ref', ref: 'payroll-cost-batches', required: true, lockedOnEdit: true },
      { key: 'externalLineId', kind: 'text', required: true, lockedOnEdit: true },
      { key: 'employeePartyId', kind: 'ref', ref: 'employees', required: true },
      { key: 'payCode', kind: 'text' },
      { key: 'component', kind: 'select', options: PAYROLL_COMPONENTS, required: true },
      { key: 'hours', kind: 'decimal' },
      { key: 'amount', kind: 'decimal', required: true },
      { key: 'memo', kind: 'text' },
    ],
  },
