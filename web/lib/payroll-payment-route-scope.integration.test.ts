...
test("payroll payment API carries the caller's scope to historical liability authorization", {
  ...
  try {
    ...
    await withBypassContext(() => db.execute(sql`insert into accounts(id,org_id,number,name,type,is_summary,is_active,eliminate,required_dimensions,custom)
    ...
    await withBypassContext(() => db.execute(sql`insert into intercompany_pairs
    ...
    await withBypassContext(() => db.execute(sql`update parties set subsidiary_id=${branchId} where org_id=${fx.orgId} and id=${fx.employeeId}`));
    const { input } = await withOrgContext(fx.orgId, () => calculatedRun(fx));
    await withOrgContext(fx.orgId, () => commitPayRun(input));
    await withBypassContext(() => db.execute(sql`update documents set status='approved' where org_id=${fx.orgId} and id=${input.documentId}`));
    const bankId = (await withOrgContext(fx.orgId, () => db.execute<{ id: string }>(sql`
    ...
    await withBypassContext(() => db.execute(sql`update journal_entries set status='posted',posted_at=now(),posted_by=${fx.actorId}
      where org_id=${fx.orgId} and id=${postedEntryId}`));
    await withBypassContext(() => db.execute(sql`update documents d set status='posted',posted_entry_id=${postedEntryId},
    ...
    await assert.rejects(
      withOrgContext(fx.orgId, () => recordPayRunPayment({ ...input, bankAccountId: bankId, allowedSubsidiaryIds: scope })),
      /pay run not found/,
    );
    ...
    const response = await withOrgContext(fx.orgId, () => POST(request(), { params: Promise.resolve({ id: input.documentId }) }));
    assert.equal(response.status, 422);
    assert.deepEqual(await response.json(), { error: "pay run not found" });
    const run = (await withOrgContext(fx.orgId, () => db.execute<{ paid_at: string | null }>(sql`
    ...
    const allowed = await withOrgContext(fx.orgId, () => POST(request(), { params: Promise.resolve({ id: input.documentId }) }));
