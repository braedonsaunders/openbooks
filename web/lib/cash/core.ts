...
/**
 * Resolve the formula engine's tax fraction from the tenant's explicit default
 * tax-rate configuration. `defaultRatePercent` belongs to the one-per-org
 * manual tax-rate provider; tax-code rows are deliberately not guessed because
 * an organization may have several jurisdictions and rate bands. The stored
 * percentage (for example, `5`) is converted to the formula fraction (`0.05`).
 */
