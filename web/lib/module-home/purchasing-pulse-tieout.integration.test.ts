  ...
  try {
    await pinClock('2026-09-17', async () => {
      ...
      await withOrgContext(org.orgId, async () => {
        ...
        assert.equal(home.apOutstanding, '4000.0000', 'open = overdue bill + future-paid bill; future-posted excluded')
        assert.equal(home.apOverdue, '1000.0000', 'only the past-due bill is overdue')
        ...
        assert.equal(hero.billedOpen, '4000.0000', 'hero billed-open ties the pulse open')
        assert.equal(hero.overdue, '1000.0000', 'hero overdue ties the pulse overdue')
        ...
        assert.equal(
          home.topExposure.reduce((sum, row) => add(sum, row.billedOpen), '0.0000'),
          home.apOutstanding,
