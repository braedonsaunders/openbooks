...
export async function createBillingRequest(
  ...
  const proj = await db.execute<{
    ...
    select id, customer_po_number
      from projects
     where id = ${input.projectId} and org_id = ${orgId}
       ${subsidiaryVisibleFilter(sql`subsidiary_id`, allowedSubsidiaryIds ?? null)}
  `);
  ...
  if (eff.billingProcedure === "application_for_payment") {
    throw new Error(
      "This project bills through applications for payment; create the invoice from its Schedule of Values workflow",
    );
  }
  if (
    input.invoiceType !== undefined &&
    input.invoiceType !== "progress" &&
    input.invoiceType !== "final"
  ) {
    throw new Error("Invoice stage must be progress or final");
  }
  const requestedBasis = input.basis ?? eff.defaultBasis;
  if (!BASES.has(requestedBasis))
    throw new Error("Choose a valid billing basis");
  if (!eff.allowedBases.includes(requestedBasis)) {
    throw new Error(
      "The selected billing basis is not enabled for this project type",
    );
  }
  if (
    requestedBasis === "field_ticket" &&
  ...
  const backupType =
    input.backupType && BACKUP_TYPES.has(input.backupType)
      ? input.backupType
      : backupRequired && BACKUP_TYPES.has(eff.backupType)
        ? eff.backupType
        : "none";
  return db.transaction(async (tx) => {
    ...
    // from turning an authorized request into an out-of-scope write.
    const lockedProject = await tx.execute<{
      ...
      select id, customer_po_number
       ...
       for share
    `);
    if (!lockedProject.rows[0]) throw new Error("Project not found");
    ...
    if (basis === "field_ticket") {
      ...
      if (selected.rows.length !== fieldTicketIds.length) {
        throw new Error(
          "Every selected Field Ticket must be approved, unbilled, and belong to this project",
        );
      }
      ...
      if (unavailable.rows.length > 0) {
        throw new Error(
          `Field Ticket ${unavailable.rows.map((row) => row.document_number).join(", ")} is already on another billing request`,
        );
      }
    }
...
export async function cancelBillingRequest(
  ...
  return db.transaction(async (tx) => {
    ...
    `);
    if (!r.rows[0])
      throw new Error("Only an open billing request can be cancelled");
    await tx.execute(sql`
      insert into audit_log (org_id, table_name, row_id, action, changes, actor_id)
      values (
        ${orgId},
