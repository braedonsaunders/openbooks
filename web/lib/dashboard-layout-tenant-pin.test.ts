...
test('quick-actions upserts pin the known tenant on the org_id/user_id conflict write', () => {
  assert.match(
    source,
    /widgets: existingLayout\.widgets \?\? \[\],[\s\S]*?quickActions,[\s\S]*?insert into user_dashboard_layouts[\s\S]*?on conflict \(org_id, user_id\)[\s\S]*?do update set[\s\S]*?where user_dashboard_layouts\.org_id = \$\{authz\.user\.orgId\}/,
  )
})
