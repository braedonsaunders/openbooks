...
An extension is one installed package of screens, record definitions, and optional backend actions. **Settings → Extensions** is its management inventory. **Apps** is the launcher people use to open installed workspaces; it does not contain a second copy of the package.
...
- **list_extensions** lists installed packages and their active versions.
...
Page customizations, navigation entries, settings, and permission definitions belong to the same extension package as its screens, records, fields, and backend actions. Every change goes through the same draft, preview, and activation flow. There is one installed version and one ownership record.
`,
}
