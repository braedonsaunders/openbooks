...
test('the HRM overview loader executes every one of its queries against a live database', { skip: !DB }, async () => {
  ...
  try {
    ...
    assert.deepEqual(data.attention, [], 'a fresh org has nothing needing attention')
    assert.equal(data.trendData.length, 12)
    assert.equal(data.trendLabels.length, 12)
    assert.equal(data.multiSubsidiary, false)
