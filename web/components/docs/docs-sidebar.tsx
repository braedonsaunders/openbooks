...
export function DocsSidebar({ categories, articles }: { categories: DocCategory[]; articles: DocNavArticle[] }) {
  ...
  const groups = useMemo(
    () =>
      categories
        .map((category) => ({
          category,
          items: filtered.filter((a) => a.category === category.key),
        }))
  ...
  return (
    <>
      <nav className="hidden h-full w-64 shrink-0 flex-col border-r border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 md:flex">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">{articleTree}</div>
      </nav>
      ...
      <details className="group shrink-0 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 md:hidden">
        ...
        <div className="border-t border-slate-200 p-3 dark:border-slate-800">
          {search}
          <nav className="mt-3 max-h-[55vh] overflow-y-auto">{articleTree}</nav>
        </div>
      </details>
    </>
  )
