...
const ACTIONS: CreateAction[] = [
  { key: 'invoice', group: 'sales', enabled: (p) => p.accountsReceivable, icon: FilePlus2, endpoint: '/api/documents/draft', body: { kind: 'customer_invoice' }, destination: (id) => `/ar?doc=${id}` },
  { key: 'creditMemo', group: 'sales', enabled: (p) => p.accountsReceivable, icon: FileMinus2, endpoint: '/api/documents/draft', body: { kind: 'customer_credit' }, destination: (id) => `/ar?doc=${id}` },
  { key: 'estimate', group: 'sales', enabled: (p) => p.accountsReceivable, icon: ClipboardList, endpoint: '/api/estimates/draft', destination: (id) => `/estimates?estimate=${id}` },
  { key: 'salesOrder', group: 'sales', enabled: (p) => p.accountsReceivable, icon: Send, endpoint: '/api/sales-orders/draft', destination: (id) => `/sales-orders?order=${id}` },
  { key: 'customerPayment', group: 'sales', enabled: (p) => p.customerPayments, icon: WalletCards, endpoint: '/api/payments/draft', body: { kind: 'customer_payment' }, destination: (id) => `/receipts?payment=${id}` },
  { key: 'bill', group: 'purchases', enabled: (p) => p.accountsPayable, icon: Receipt, endpoint: '/api/documents/draft', body: { kind: 'vendor_bill' }, destination: (id) => `/ap?doc=${id}` },
  { key: 'vendorCredit', group: 'purchases', enabled: (p) => p.accountsPayable, icon: FileMinus2, endpoint: '/api/documents/draft', body: { kind: 'vendor_credit' }, destination: (id) => `/ap?doc=${id}` },
  ...
  { key: 'deposit', group: 'accounting', enabled: (p) => p.journal, icon: Landmark, endpoint: '/api/documents/draft', body: { kind: 'deposit' }, destination: (id) => `/banking/transactions?doc=${id}` },
  { key: 'transfer', group: 'accounting', enabled: (p) => p.journal, icon: Send, endpoint: '/api/documents/draft', body: { kind: 'transfer' }, destination: (id) => `/banking/transactions?doc=${id}` },
  ...
  { key: 'customer', group: 'peopleAndLists', enabled: (p) => p.parties, icon: UsersRound, endpoint: '/api/parties/draft', body: { role: 'customer' }, destination: (id) => `/entities/customers?party=${id}` },
  { key: 'vendor', group: 'peopleAndLists', enabled: (p) => p.parties, icon: Building2, endpoint: '/api/parties/draft', body: { role: 'vendor' }, destination: (id) => `/entities/vendors?party=${id}` },
  { key: 'employee', group: 'peopleAndLists', enabled: (p) => p.parties, icon: UserRound, endpoint: '/api/parties/draft', body: { role: 'employee' }, destination: (id) => `/entities/employees?party=${id}` },
