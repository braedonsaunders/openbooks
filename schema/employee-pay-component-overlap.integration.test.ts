 ...
 *
 * Each test leases its own scratch org and component. The integration
 * partition drains outstanding scratch-org leases at every top-level test
 * boundary (scripts/test-fixture-lifecycle.mjs), so a harness memoized
 * across tests would write into a released slot: the second test then
 * fails with a 23503 FK on a component that no longer exists. Leasing per
 * test keeps every insert against a live fixture.
 */
...
type Harness = {
  ...
  dropScratchOrg: EngineFixtures["dropScratchOrg"];
  org: Awaited<ReturnType<EngineFixtures["createScratchOrg"]>>;
  componentId: string;
};

async function setup(): Promise<Harness> {
  const [{ db, withBypassContext }, { createScratchOrg, dropScratchOrg }, { seedPayrollComponents }] = await Promise.all([
  ...
  try {
    ...
    assert.ok(component, "seeded CA pack provides a BASE component");
    return { db, withBypassContext, dropScratchOrg, org, componentId: component.id };
  } catch (error) {
...
async function insertEmployment(h: Harness, partyId: string, subsidiaryId: string): Promise<string> {
  const id = randomUUID();
...
test("an overlapping active assignment for the same employment is refused", { skip: !DB }, async () => {
  const h = await setup();
  try {
    const employmentId = await insertEmployment(h, h.org.customerId, h.org.subsidiaryId);
    ...
    await h.dropScratchOrg(h.org.orgId);
  }
});
...
test("not-yet-stamped rows are guarded, but a second employment keeps its own window", { skip: !DB }, async () => {
  ...
  try {
    ...
    const first = await insertEmployment(h, h.org.customerId, h.org.subsidiaryId);
    const second = await insertEmployment(h, h.org.customerId, h.org.subsidiaryId);
