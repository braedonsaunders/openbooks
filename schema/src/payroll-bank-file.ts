...
export const payRunBankFiles = pgTable(
  "pay_run_bank_files",
  {
    ...
    format: text("format", { enum: ["cpa005", "nacha", "sepa", "cemtex", "bacs", "zengin", "cnab240"] }).notNull(),
  ...
  (t) => [
    ...
    // single-character file ID modifier. sepa, cemtex, bacs, zengin and
    // cnab240 carry neither: the artifact writer
    // (engine/src/payroll/bank-file-artifact.ts) allocates a number ONLY for
    // cpa005 and a modifier ONLY for nacha, so both columns stay NULL for
    // every other format, whose traceable identity is the sequence_value
    // allocation (SEPA's message id; the Bacs VOL1 serial and UHL1 file
    // number live in the file bytes, derived from that same allocation — as
    // do zengin's bank-facing identity and cnab240's NSA arquivo sequence).
    // One honest arm per format — never a catch-all, so the cpa005/nacha
    // arms keep checking.
    check(
      ...
      sql`(format = 'cpa005' and file_creation_number between 1 and 9999 and file_id_modifier is null)
          ...
          or (format = 'zengin' and file_creation_number is null and file_id_modifier is null)
          or (format = 'cnab240' and file_creation_number is null and file_id_modifier is null)`,
    ),
  ],
);
