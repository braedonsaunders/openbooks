...
--
-- Two branches on the stock-count lifecycle (U14, restaged bytes b3e411056).
-- Posted and cancelled counts are immutable in the engine, so a negative
-- observation on one can never be re-recorded: negatives on counts still
-- open for correction REFUSE with a re-record remedy, while negatives on
-- posted or cancelled counts are a NOTICE naming grandfathered legacy
-- (marked is_pre_guard_legacy by the migration, recorded in
-- upgrade_legacy_provenance by 0326). Zero rows means ready for 0299.
WITH flagged AS (
...
SELECT * FROM (
  SELECT '0299.negative_counted_quantity' AS code,
    ...
    FROM flagged f
   WHERE f.status NOT IN ('posted', 'cancelled')
...
SELECT * FROM (
  SELECT '0299.negative_counted_quantity_grandfathered' AS code,
         'notice' AS severity,
         format('immutable count %s (%s) line %s (item %s at stock location %s) counted %s',
                f.stock_count_id, f.status, f.id, f.item_id, f.stock_location_id,
                f.counted_quantity) AS subject,
         format('stock_count_lines row %s in org %s carries a negative counted_quantity %s on a %s count. The phantom variance stands as history, marked is_pre_guard_legacy and recorded as grandfathered legacy provenance',
                f.id, f.org_id, f.counted_quantity, f.status) AS detail,
         'No re-record is possible: posted and cancelled counts are immutable — correct variances with a new count. These rows are recorded as grandfathered legacy.' AS remedy
    FROM flagged f
   WHERE f.status IN ('posted', 'cancelled')
   ORDER BY f.stock_count_id, f.id
   LIMIT 20
) notice_rows;
