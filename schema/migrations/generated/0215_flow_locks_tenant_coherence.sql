-- OpenBooks forward migration 0215_flow_locks_tenant_coherence.
--
-- The canonical baseline installed flow_locks_flow_id_fkey as
...
-- flow belongs to the lock's organization. flows is org-scoped
...
-- flow. A lock for organization A can therefore name a flow owned by
-- organization B.
--
...
DO $flow_locks_tenant_preflight$
DECLARE
  violation record;
BEGIN
  SELECT lock.ctid::text AS child_ctid,
         lock.org_id::text AS child_org_id,
         lock.flow_id::text AS flow_id,
         flow.org_id::text AS referenced_org_id
    INTO violation
    FROM public.flow_locks lock
    LEFT JOIN public.flows flow
      ON flow.id = lock.flow_id
   WHERE lock.flow_id IS NOT NULL
     AND (flow.id IS NULL OR flow.org_id IS DISTINCT FROM lock.org_id)
   ORDER BY lock.ctid
   LIMIT 1;
  ...
  IF FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'legacy data violates tenant coherence: public.flow_locks.flow_id',
      ...
      HINT = 'Reconcile the flow lock flow_id to a flow owned by the same organization, then retry migration 0215; this migration will not rewrite financial history.';
  END IF;
END
$flow_locks_tenant_preflight$;
...
COMMENT ON CONSTRAINT flow_locks_flow_id_fkey
  ON public.flow_locks IS
  'openbooks:flow_locks.flow_tenant_coherence:v1 - flow references must remain within the lock organization; same-tenant parent deletion cascades';
