...
-- The legacy line guard deliberately rejects edits to approved and pending
-- scenarios.  This migration is the one sanctioned historical owner-fill;
-- keep those scenarios intact while the entity is attached, then restore the
-- guard before any application writer can run.
ALTER TABLE public.budget_lines DISABLE TRIGGER budget_line_guard;

DO $budget_subsidiary_backfill$
DECLARE
  missing_root record;
BEGIN
  -- Never guess an entity when the tenant's root is absent or ambiguous.
  -- Aborting leaves the table untouched so an operator can repair the tenant
  -- and retry.  The exact-one check also makes UPDATE ... FROM deterministic
  -- when a legacy tenant has an invalid duplicate-root hierarchy.
  SELECT bl.org_id, count(s.id) AS root_count
    INTO missing_root
    FROM public.budget_lines bl
    LEFT JOIN public.subsidiaries s
      ON s.org_id = bl.org_id
  ...
  IF FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'budget subsidiary backfill found a tenant without exactly one active root subsidiary',
      DETAIL = jsonb_build_object('org_id', missing_root.org_id, 'active_root_count', missing_root.root_count)::text,
      HINT = 'Create exactly one active non-elimination root subsidiary, then retry migration 0079; no budget rows were rewritten.';
  END IF;
...
ALTER TABLE public.budget_lines ENABLE TRIGGER budget_line_guard;
