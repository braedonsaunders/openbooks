-- OpenBooks forward migration 0208_pay_run_adjustments_run_tenant_coherence.
--
-- The canonical baseline installed pay_run_adjustments_run_fkey as
...
-- pay run belongs to the adjustment's organization. pay_runs is org-scoped
...
-- adjustment can therefore store org A with an org-B pay_run document_id.
--
...
DO $pay_run_adjustments_run_tenant_preflight$
DECLARE
  violation record;
BEGIN
  SELECT adjustment.ctid::text AS child_ctid,
         adjustment.org_id::text AS child_org_id,
         adjustment.pay_run_document_id::text AS pay_run_document_id,
         pay_run.org_id::text AS referenced_org_id
    INTO violation
    FROM public.pay_run_adjustments adjustment
    LEFT JOIN public.pay_runs pay_run
      ON pay_run.document_id = adjustment.pay_run_document_id
   WHERE adjustment.pay_run_document_id IS NOT NULL
     AND (pay_run.document_id IS NULL OR pay_run.org_id IS DISTINCT FROM adjustment.org_id)
  ...
  IF FOUND THEN
    RAISE EXCEPTION USING
      ERRCODE = '23514',
      MESSAGE = 'legacy data violates tenant coherence: public.pay_run_adjustments.pay_run_document_id',
      ...
      HINT = 'Reconcile the pay-run adjustment pay_run_document_id to a pay run owned by the same organization, then retry migration 0208; this migration will not rewrite financial history.';
  END IF;
END
$pay_run_adjustments_run_tenant_preflight$;
...
ALTER TABLE public.pay_run_adjustments
  VALIDATE CONSTRAINT pay_run_adjustments_run_fkey;

COMMENT ON CONSTRAINT pay_run_adjustments_run_fkey
  ON public.pay_run_adjustments IS
  'openbooks:pay_run_adjustments.run_tenant_coherence:v1 - pay run references must remain within the adjustment organization; same-tenant parent deletion cascades';
