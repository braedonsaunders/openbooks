...
test("the US pack still refuses a withholding state, and an unknown one differently", () => {
  ...
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "HI"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "ID"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "KS"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "LA"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "MO"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "MT"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "ND"));
  assert.doesNotThrow(() => assertPayrollRegionSupported("US", "OK"));
  assert.throws(
  ...
  // Forty-seven: the thirty-eight states whose income tax the pack computes
  ...
  // cannot disagree.
  assert.equal(
    PAYROLL_COUNTRY_PACKS.US!.regions.supported.length, 47,
    "thirty-eight states with an engine, nine with no wage income tax",
  );
});
