    ...
    return db.transaction(async (tx) => {
      await tx.execute(sql`
        update qbd_captures set status = 'cancelled', finished_at = now(),
         ...
         where connection_id = ${input.connectionId} and status in ('queued', 'running')`);
      await tx.execute(sql`
        update qbd_requests r set status = 'cancelled', updated_at = now()
         where r.connection_id = ${input.connectionId} and r.status in ('queued', 'sent')
           and exists (select 1 from qbd_captures c where c.id = r.capture_id and c.status = 'cancelled')`);
      ...
      await withOrgContext(orgId, () => db.transaction(async (tx) => {
        await tx.execute(sql`
          update qbd_captures set status = 'failed', error_message = 'Web Connector capture timed out',
                 finished_at = now(), updated_at = now() where id = ${captureId} and status in ('queued', 'running')`);
        await tx.execute(sql`
          update qbd_requests set status = 'cancelled', updated_at = now()
           where capture_id = ${captureId} and status in ('queued', 'sent')`);
      }));
  ...
  await withOrgContext(orgId, async () => {
    await db.execute(sql`
      update qbd_requests set response_xml = null, updated_at = now()
       where capture_id = ${captureId} and response_xml is not null`);
  });
}
    ...
    await tx.execute(sql`
      update qbd_requests r set status = 'cancelled', session_id = null, updated_at = now()
       where r.status in ('queued', 'sent')
         and exists (select 1 from qbd_captures c where c.id = r.capture_id and c.expires_at < now())`);
    await tx.execute(sql`
...
export async function nextWebConnectorRequest(ticket: string, metadata: {
  ...
  return withBypassContext(async () => db.transaction(async (tx) => {
    if (metadata.country && current.expectedRegion && metadata.country.toUpperCase() !== current.expectedRegion.toUpperCase()) {
      ...
      await tx.execute(sql`update qbd_sessions set status = 'error', last_error = ${error}, closed_at = now(), last_seen_at = now() where id = ${ticket}`);
      await tx.execute(sql`
        update qbd_captures set status = 'failed', error_message = ${error}, finished_at = now(), updated_at = now()
         where connection_id = ${current.connectionId} and status in ('queued', 'running')`);
      await tx.execute(sql`
        update qbd_requests r set status = 'cancelled', session_id = null, updated_at = now()
         where r.connection_id = ${current.connectionId} and r.status in ('queued', 'sent')
           and exists (select 1 from qbd_captures c where c.id = r.capture_id and c.status = 'failed')`);
      return "";
    }
    await tx.execute(sql`
      ...
      update qbd_requests set status = 'sent', session_id = ${ticket}, sent_at = now(), updated_at = now()
       where id = (
         select r.id from qbd_requests r
          join qbd_captures c on c.id = r.capture_id
...
export async function acceptWebConnectorResponse(ticket: string, responseXml: string, hresult: string, message: string): Promise<number> {
  ...
  return withBypassContext(async () => db.transaction(async (tx) => {
    ...
    if (hresult || !responseXml.trim()) {
      ...
      await tx.execute(sql`update qbd_requests set status = 'failed', error_message = ${error}, completed_at = now(), updated_at = now() where id = ${request.id}`);
      await tx.execute(sql`update qbd_captures set status = 'failed', error_message = ${error}, finished_at = now(), updated_at = now() where id = ${request.captureId}`);
      await tx.execute(sql`update qbd_requests set status = 'cancelled', updated_at = now() where capture_id = ${request.captureId} and status in ('queued', 'sent') and id <> ${request.id}`);
      await tx.execute(sql`update qbd_sessions set last_error = ${error}, last_seen_at = now() where id = ${ticket}`);
      return -101;
    }
    ...
    await tx.execute(sql`
      update qbd_requests set status = 'complete', response_xml = ${responseXml}, response_sha256 = ${hash},
             completed_at = now(), updated_at = now() where id = ${request.id}`);
    ...
    await tx.execute(sql`
      update qbd_captures set status = ${complete ? "complete" : "running"},
       ...
       where id = ${request.captureId}`);
    await tx.execute(sql`update qbd_sessions set last_seen_at = now() where id = ${ticket}`);
...
export async function closeWebConnectorSession(ticket: string): Promise<string> {
  ...
  return withBypassContext(async () => {
    await db.transaction(async (tx) => {
      await tx.execute(sql`update qbd_sessions set status = 'closed', closed_at = now(), last_seen_at = now() where id = ${ticket}`);
      await tx.execute(sql`
        update qbd_requests r set
          ...
          from qbd_captures c where c.id = r.capture_id and r.session_id = ${ticket} and r.status = 'sent'`);
    });
