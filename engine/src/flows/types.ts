...
export interface FlowSubjectAdapter {
  ...
  /**
   * True when this adapter's releaseApproval delegates to a registered
   * handler (releaseFlowApproval) instead of releasing inline — i.e. the
   * subject kind needs a handler registered at process boot (engine
   * handlers via installEngineSeams, product handlers at web boot), or
   * its gates strand pending forever. The guard test derives the full
   * set from the subject registry via handlerReleasedSubjectKinds().
   */
  releaseViaHandler?: boolean;
  /**
