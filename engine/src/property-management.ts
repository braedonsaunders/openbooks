...
export async function finalizeCamPool(orgId: string, actorId: string, poolId: string): Promise<{ actualAmount: string; allocations: number }> {
  return db.transaction(async (tx) => {
    ...
    const sourceFingerprint = { lines: actual.rows[0]!.lines, total: actual.rows[0]!.amount, lastChange: actual.rows[0]!.last_change };
    ...
    // to both. Re-reading the exact source predicate under a fresh statement
    // snapshot immediately before committing turns any residual change into a
    // refused finalization instead of stale immutable actuals.
    ...
    await audit(tx, orgId, "cam_pools", poolId, "finalize", actorId, {
      after: { status: "finalized", actual_amount: actualAmount },
      actualAmount,
      allocationCount: weighted.length,
      locationId: pool.location_id,
      expenseAccountIds: pool.expense_account_ids,
      periodStartsOn: pool.period_starts_on,
      periodEndsOn: pool.period_ends_on,
      sourceFingerprint,
      coveredScopes: coveredScopes.rows.map((scope) => ({ periodId: scope.period_id, bookId: scope.book_id })),
    });
