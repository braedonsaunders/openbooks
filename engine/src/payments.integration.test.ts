...
test("foreign-currency payment, application, realized FX, evidence, and reversal are atomic", { skip: !DB }, async () => {
  ...
  try {
    ...
    await updateDraftPayment(payment.id, {
      allocations: [{ openLineId: targetLineId, amount: "100" }],
      bankAccountId: org.accounts.bank,
    }, userId);
    ...
    const evidence = (await db.execute(sql`
      select a.amount, a.source_amount, a.transaction_amount, a.transaction_currency,
    ...
    `)) as unknown as { rows: {
      amount: string; source_amount: string; transaction_amount: string; transaction_currency: string;
