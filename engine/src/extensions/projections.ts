...
export async function getExtensionSettings(orgId: string, tx: SqlExecutor = db, asOf?: Date): Promise<Record<string, Record<string, unknown>>> {
  ...
  if (asOf) {
    ...
    const history = (row?.settings.extensionSettingsHistory ?? {}) as SettingHistory;
    return Object.fromEntries(Object.entries(history).map(([extensionKey, settings]) => [extensionKey,
  ...
  const settings = row?.settings.extensionSettings;
...
function appendSettingHistory(settings: Record<string, unknown>, extensionKey: string, key: string, value: unknown, effectiveFrom: string) {
  const history = structuredClone((settings.extensionSettingsHistory ?? {}) as SettingHistory);
  const module = history[extensionKey] ?? {};
  module[key] = [...(module[key] ?? []), { effectiveFrom, value }];
  history[extensionKey] = module;
  settings.extensionSettingsHistory = history;
}
...
export async function projectSupplementalContributions(tx: SqlExecutor, args: {
  ...
  const previous = args.previousVersionId ? (await tx.execute<{ manifest: { contributions?: unknown[] } }>(sql`select manifest from app_versions where org_id = ${args.orgId} and app_id = ${args.extensionId} and id = ${args.previousVersionId}`)).rows[0]?.manifest.contributions ?? [] : [];
  ...
  const siblings = (await listActiveExtensionContributions(args.orgId, tx)).filter((entry) => entry.extensionId !== args.extensionId);
  for (const contribution of args.contributions) {
    if (contribution.kind === 'permission') {
      const owner = (await tx.execute<{ key: string }>(sql`
        select m.key from apps m join app_versions v on v.app_id = m.id and v.org_id = m.org_id
        cross join lateral jsonb_array_elements(coalesce(v.manifest->'contributions', '[]'::jsonb)) declaration
        where m.org_id = ${args.orgId} and m.id <> ${args.extensionId}
          and declaration->>'kind' = 'permission' and declaration->>'key' = ${contribution.key} limit 1
      `)).rows[0];
      if (owner) throw new ExtensionProjectionError(`permission ${contribution.key} is reserved by extension ${owner.key}; historical grants cannot be reassigned`);
    }
    if (contribution.kind === 'permission' && (isCataloguePermission(contribution.key) || siblings.some((entry) => entry.contribution.kind === 'permission' && entry.contribution.key === contribution.key))) {
      throw new ExtensionProjectionError(`permission ${contribution.key} already has an owner`);
    }
  }
  ...
  for (const nav of navs) if (nav.requiredPermission && !isCataloguePermission(nav.requiredPermission) &&
    ...
    throw new ExtensionProjectionError(`navigation requires undeclared permission ${nav.requiredPermission}`);
  }
  ...
  if (settings.length) {
    ...
    const modules = await getExtensionSettings(args.orgId, tx);
    const values = { ...(modules[args.extensionKey] ?? {}) };
    const after: Record<string, unknown> = { ...row.settings, extensionSettings: { ...modules, [args.extensionKey]: values } };
    ...
    for (const setting of settings) if (!Object.hasOwn(values, setting.key) && setting.defaultValue !== undefined) {
      ...
      appendSettingHistory(after, args.extensionKey, setting.key, setting.defaultValue, effectiveFrom);
    }
  ...
  for (const contribution of args.contributions.filter((c) => c.kind === 'permission')) {
    ...
    await audit(tx, { ...args, rowId: args.extensionId, table: 'apps', before: prior ? { contribution: prior, versionId: args.previousVersionId, grantable: true } : null, after: { contribution, versionId: args.versionId, grantable: true } });
  }
  for (const prior of previousDeclarations.filter((entry) => entry.kind !== 'nav')) {
    ...
    await audit(tx, { ...args, rowId: args.extensionId, table: 'apps', event: 'extension_projection_withdrawn', before: { contribution: prior, active: true }, after: { contribution: prior, active: false, storedValuesPreserved: true } });
  }
}

export async function withdrawSupplementalContributions(tx: SqlExecutor, args: {
  orgId: string; actorId: string; extensionId: string; extensionKey: string; reason: string;
}) {
  const active = (await listActiveExtensionContributions(args.orgId, tx)).filter((entry) => entry.extensionId === args.extensionId);
  ...
  for (const entry of active.filter((entry) => entry.contribution.kind !== 'nav')) await audit(tx, {
    ...args, rowId: args.extensionId, table: 'apps', event: 'extension_projection_withdrawn',
...
export async function updateExtensionSetting(args: {
  orgId: string; actorId: string; extensionKey: string; key: string; value: unknown; reason: string; effectivePermissions: readonly string[];
  expectedValue?: unknown; expectedExtensionVersionId?: string;
}) {
  ...
  return db.transaction(async (tx) => {
    await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${`extension-projections:${args.orgId}`}, 0))`);
    if (!(await actorHasPermission(tx, args.orgId, args.actorId, 'admin.setup.manage'))) throw new Error('admin.setup.manage required');
    const entry = (await listActiveExtensionContributions(args.orgId, tx)).find((entry) => entry.extensionKey === args.extensionKey && entry.contribution.kind === 'setting' && entry.contribution.key === args.key);
    if (!entry || entry.contribution.kind !== 'setting') throw new Error('Active extension setting not found');
    if (args.expectedExtensionVersionId !== undefined && entry.versionId !== args.expectedExtensionVersionId) throw Object.assign(new Error('Extension version changed; reload the setting before saving'), { status: 409 });
    ...
    const row = (await tx.execute<{ settings: Record<string, unknown> }>(sql`select settings from orgs where id = ${args.orgId} for update`)).rows[0]!;
    const values = await getExtensionSettings(args.orgId, tx);
    if (Object.hasOwn(args, 'expectedValue') && !isDeepStrictEqual(values[args.extensionKey]?.[args.key] ?? null, args.expectedValue ?? null)) throw Object.assign(new Error('Setting changed; reload before saving'), { status: 409 });
    const after: Record<string, unknown> = { ...row.settings, extensionSettings: { ...values, [args.extensionKey]: { ...values[args.extensionKey], [args.key]: args.value } } };
    appendSettingHistory(after, args.extensionKey, args.key, args.value, await effectiveTimestamp(tx));
    await tx.execute(sql`update orgs set settings = ${JSON.stringify(after)}::jsonb where id = ${args.orgId}`);
    await audit(tx, { ...args, table: 'orgs', rowId: args.orgId, event: 'extension_setting_changed', before: row.settings, after });
    return { ok: true };
  });
}
