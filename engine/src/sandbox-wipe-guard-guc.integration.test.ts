...
const migration = readFileSync(
  new URL("../../schema/migrations/generated/0019_sandbox_wipe_guard_guc.sql", import.meta.url),
  "utf8",
);

const legacyWipGuard = `
...
test("0019 upgrades the published WIP guard body in place", { skip: !DB }, async () => {
  const client = await pool.connect();
  try {
    await client.query("begin");
    await client.query(legacyWipGuard);
    ...
    assert.match(before.rows[0]!.definition, /current_setting\('app\.sandbox_wipe'/);

    await client.query(migration);
    const after = await client.query<{ definition: string }>(
      "select pg_get_functiondef('public.wip_prebill_event_append_only_guard()'::regprocedure) as definition",
    );
    assert.match(after.rows[0]!.definition, /current_setting\('openbooks\.sandbox_wipe'/);
    assert.doesNotMatch(after.rows[0]!.definition, /current_setting\('app\.sandbox_wipe'/);
  } finally {
