...
async function runControlledRepairInvariants(): Promise<void> {
  const marker = new Date().toISOString().replace(/\D/g, "").slice(0, 17);
  const reportsDir = join(runtimeDir, "repair-reports");
  mkdirSync(reportsDir, { recursive: true });
  const runRepair = (
    script: string,
    flags: string[],
  ): ReturnType<typeof spawnSync> =>
    spawnSync("npx", ["tsx", script, ...flags], {
      cwd: repoRoot,
      env: { ...process.env },
      encoding: "utf8",
  ...
  const taxOrg = await createScratchOrg();
  const taxActors = await seedFlowActors(taxOrg.orgId);
  try {
    ...
    await db.execute(sql`
      insert into projects
        (id, org_id, subsidiary_id, code, name, customer_id, status,
         is_active, custom)
      values
        (${projectId}, ${taxOrg.orgId}, ${taxOrg.subsidiaryId},
         'TAX-REPAIR', 'Tax repair project', ${taxOrg.customerId}, 'active',
         true, '{}'::jsonb)
    `);
    await db.execute(sql`
      insert into tax_codes
        (id, org_id, code, name, calculation_type, collected_account_id,
         paid_account_id, recoverable_percent, is_active)
      values
        (${taxCodeId}, ${taxOrg.orgId}, 'REPAIR-TAX', 'Repair tax',
         'standard', ${taxOrg.accounts.taxOutput}, ${taxOrg.accounts.taxInput},
         '100', true)
    `);
    await db.execute(sql`
      insert into documents
        (id, org_id, kind, status, document_number, subsidiary_id, party_id,
         document_date, posting_date, currency, fx_rate, subtotal, tax_total,
         total, created_by, updated_by)
      values
        (${documentId}, ${taxOrg.orgId}, 'customer_invoice', 'approved',
         'TAX-REPAIR-001', ${taxOrg.subsidiaryId}, ${taxOrg.customerId},
         ${taxOrg.date}, ${taxOrg.date}, 'CAD', '1', '100', '10', '110',
         ${taxActors.adminId}, ${taxActors.adminId})
    `);
    await db.transaction(async (tx) => {
      ...
      await tx.execute(sql`
        insert into journal_entries
          (id, org_id, book_id, subsidiary_id, entry_number, posting_date,
           ...
           updated_by, posted_at, posted_by)
        values
          (${sourceEntryId}, ${taxOrg.orgId}, ${taxOrg.bookId},
           ${taxOrg.subsidiaryId}, 'TAX-REPAIR-001', ${taxOrg.date},
           ${taxOrg.periodId}, 'Historical tax dimension defect', 'posted',
           'migration', ${documentId}, ${taxActors.adminId},
           ${taxActors.adminId}, now(), ${taxActors.adminId})
      `);
      await tx.execute(sql`
        insert into journal_lines
          (org_id, entry_id, line_number, account_id, subsidiary_id, amount,
           currency, txn_amount, fx_rate, project_id, tax_code_id,
           is_open_item)
        values
          (${taxOrg.orgId}, ${sourceEntryId}, 1, ${taxOrg.accounts.ar},
           ${taxOrg.subsidiaryId}, '110', 'CAD', '110', '1', null, null, true),
          (${taxOrg.orgId}, ${sourceEntryId}, 2, ${taxOrg.accounts.revenue},
           ${taxOrg.subsidiaryId}, '-100', 'CAD', '-100', '1', null, null,
           false),
          (${taxOrg.orgId}, ${sourceEntryId}, 3, ${taxOrg.accounts.taxOutput},
           ${taxOrg.subsidiaryId}, '-10', 'CAD', '-10', '1', ${projectId},
           ${taxCodeId}, false)
      `);
      await tx.execute(sql`
        insert into journal_entries
          (id, org_id, book_id, subsidiary_id, entry_number, posting_date,
           ...
           reverses_entry_id, created_by, updated_by, posted_at, posted_by)
        values
          (${reversalEntryId}, ${taxOrg.orgId}, ${taxOrg.bookId},
           ${taxOrg.subsidiaryId}, 'TAX-REPAIR-001-R', ${taxOrg.date},
           ${taxOrg.periodId}, 'Historical tax dimension reversal', 'posted',
           'migration', ${documentId}, ${sourceEntryId}, ${taxActors.adminId},
           ${taxActors.adminId}, now(), ${taxActors.adminId})
      `);
      await tx.execute(sql`
        insert into journal_lines
          (org_id, entry_id, line_number, account_id, subsidiary_id, amount,
           ...
           is_open_item)
        values
          (${taxOrg.orgId}, ${reversalEntryId}, 1, ${taxOrg.accounts.ar},
           ${taxOrg.subsidiaryId}, '-110', 'CAD', '-110', '1', null, null,
           false),
          (${taxOrg.orgId}, ${reversalEntryId}, 2, ${taxOrg.accounts.revenue},
           ${taxOrg.subsidiaryId}, '100', 'CAD', '100', '1', null, null,
           false),
          (${taxOrg.orgId}, ${reversalEntryId}, 3,
           ${taxOrg.accounts.taxOutput}, ${taxOrg.subsidiaryId}, '10', 'CAD',
           '10', '1', ${projectId}, ${taxCodeId}, false)
      `);
      await tx.execute(sql`
        update journal_entries
           set status = 'reversed', updated_at = now(),
               updated_by = ${taxActors.adminId}
         where id = ${sourceEntryId}
      `);
      await tx.execute(sql`
        update documents
           set status = 'voided', posted_entry_id = ${sourceEntryId},
               reversal_entry_id = ${reversalEntryId}, voided_at = now(),
               voided_by = ${taxActors.adminId},
               void_reason = 'Historical repair invariant fixture',
               open_balance = null, updated_at = now(),
               updated_by = ${taxActors.adminId}
         where id = ${documentId}
      `);
    });
    await db.execute(sql`
      insert into period_locks
        (org_id, period_id, book_id, subsidiary_id, module, state, locked_at,
         locked_by, reason, created_by, updated_by)
      values
        (${taxOrg.orgId}, ${taxOrg.periodId}, ${taxOrg.bookId},
         ${taxOrg.subsidiaryId}, 'gl', 'closed', now(), ${taxActors.adminId},
         'Controlled repair closed-period fixture', ${taxActors.adminId},
         ${taxActors.adminId})
    `);
    const taxPlanPath = join(
      reportsDir,
      `repair-tax-project-${marker}-plan.json`,
    );
    const taxApplyPath = join(
      reportsDir,
      `repair-tax-project-${marker}-apply.json`,
    );
    const taxPlan = runRepair(
      "engine/src/validation/repair-posted-tax-project-dimensions.ts",
      [
        `--org=${taxOrg.orgId}`,
        "--production",
        `--out=${taxPlanPath}`,
    ...
    if (taxPlan.status !== 0) {
      throw new Error(
        `tax dimension repair plan failed: ${taxPlan.stderr || taxPlan.stdout}`,
      );
    }
    const planned = JSON.parse(readFileSync(taxPlanPath, "utf8")) as any;
    if (
      planned.population.candidateLines !== 2 ||
      planned.population.reversalPairViolations !== 0 ||
      planned.population.closedScopes !== 1 ||
      planned.blockingCount !== 1
    ) {
      throw new Error("tax dimension repair preflight did not expose controls");
    }
    const taxApply = runRepair(
      "engine/src/validation/repair-posted-tax-project-dimensions.ts",
      [
        `--org=${taxOrg.orgId}`,
        "--production",
        "--apply",
        "--controlled-reopen",
        `--actor=${taxActors.adminId}`,
        `--reopen-requester=${taxActors.submitterId}`,
        `--reopen-approver=${taxActors.adminId}`,
        "--reason=Controller approved exact tax-control dimension remediation",
        `--out=${taxApplyPath}`,
      ],
    );
    if (taxApply.status !== 0) {
      throw new Error(
        `tax dimension repair apply failed: ${taxApply.stderr || taxApply.stdout}`,
      );
    }
    const applied = JSON.parse(readFileSync(taxApplyPath, "utf8")) as any;
    const taxState = await one<{
      candidates: number;
      line_audits: number;
      ...
      select
        count(*) filter (
          where line.tax_code_id = ${taxCodeId}
            and line.project_id is not null
        )::int as candidates,
        (
          select count(*)::int
            from audit_log audit
           where audit.org_id = ${taxOrg.orgId}
             and audit.table_name = 'journal_lines'
             and audit.changes->>'mode' =
                 'tax_control_dimension_correction'
        ) as line_audits,
        (
          select count(*)::int
            from audit_log audit
           where audit.org_id = ${taxOrg.orgId}
             ...
             and audit.request_id like 'tax-project-dimension-repair:%'
        ) as document_audits,
        (
          select count(*)::int
            from period_locks lock
           where lock.org_id = ${taxOrg.orgId}
             and lock.period_id = ${taxOrg.periodId}
             and lock.book_id = ${taxOrg.bookId}
             and lock.subsidiary_id = ${taxOrg.subsidiaryId}
             and lock.module = 'gl'
             and lock.state = 'closed'
        ) as closed_locks,
        coalesce(sum(line.amount), 0)::text as net
        from journal_lines line
       where line.org_id = ${taxOrg.orgId}
         and line.entry_id in (${sourceEntryId}, ${reversalEntryId})
    `);
    const taxOk =
      applied.invariants.projectDimensionsCleared === 2 &&
      applied.invariants.protectedFinancialFieldsChanged === 0 &&
      applied.invariants.controlledReopenUsed === true &&
      applied.invariants.controlledReopenRequests.every(
        (request: { status: string }) => request.status === "reclosed",
      ) &&
      taxState.candidates === 0 &&
      taxState.line_audits === 2 &&
      taxState.document_audits === 1 &&
      taxState.closed_locks === 1 &&
      taxState.net === "0.0000";
    saveJson(
      join(evidenceDir, `repair-tax-project-${marker}.json`),
      {
        checkpoint: `repair-tax-project-${marker}`,
        plan: {
          candidateLines: planned.population.candidateLines,
          closedScopes: planned.population.closedScopes,
          reversalPairViolations:
            planned.population.reversalPairViolations,
          fingerprint: planned.population.populationFingerprint,
        },
        apply: applied.invariants,
        state: taxState,
        comparison: { ok: taxOk },
      },
    );
    if (!taxOk) throw new Error("tax dimension repair invariants failed");
    console.log(`PASS repair-tax-project-${marker}`);
  } finally {
    await dropScratchOrg(taxOrg.orgId);
  }

  const provenanceOrg = await createScratchOrg();
  const untouchedOrg = await createScratchOrg();
  const provenanceActors = await seedFlowActors(provenanceOrg.orgId);
  const untouchedActors = await seedFlowActors(untouchedOrg.orgId);
  const seedProvenanceCandidate = async (
    ...
    await db.execute(sql`
      update accounts
         set custom = jsonb_set(custom, '{nsId}', '"A1"'::jsonb)
       where org_id = ${org.orgId} and id = ${org.accounts.revenue}
    `);
    await db.execute(sql`
      update items
         set custom = jsonb_set(custom, '{nsId}', '"I1"'::jsonb)
    ...
    await db.execute(sql`
      insert into documents
        (id, org_id, kind, status, document_number, subsidiary_id, party_id,
         document_date, posting_date, currency, fx_rate, subtotal, tax_total,
         total, custom, created_by, updated_by)
      values
        (${documentId}, ${org.orgId}, 'customer_invoice', 'approved',
         ${`PROVENANCE-${documentRef}`}, ${org.subsidiaryId}, ${org.customerId},
         ${org.date}, ${org.date}, 'CAD', '1', '100', '0', '100',
         ${JSON.stringify({ nsId: documentRef })}::jsonb, ${actorId},
         ${actorId})
    `);
    await db.execute(sql`
      insert into document_lines
        (id, org_id, document_id, line_number, account_id, item_id, quantity,
         unit_price, amount, tax_amount, tax_input_amount, custom)
      values
        (${lineId}, ${org.orgId}, ${documentId}, 1, ${org.accounts.revenue},
         null, '1', '100', '100', '0', '0', '{}'::jsonb)
    `);
    await postDocument(documentId, {
      control: {
        ar: org.accounts.ar,
  ...
  try {
    const targetCandidate = await seedProvenanceCandidate(
      provenanceOrg,
      provenanceActors.adminId,
      "D1",
    );
    const untouchedCandidate = await seedProvenanceCandidate(
      untouchedOrg,
      untouchedActors.adminId,
      "D1",
    );
    const sourcePath = join(
      reportsDir,
      `repair-commercial-provenance-${marker}-source.json`,
    );
    saveJson(sourcePath, [
      {
        mainline: "F",
    ...
    const planPath = join(
      reportsDir,
      `repair-commercial-provenance-${marker}-plan.json`,
    );
    const applyPath = join(
      reportsDir,
      `repair-commercial-provenance-${marker}-apply.json`,
    );
    const plan = runRepair(
      "engine/src/validation/repair-invoice-line-commercial-provenance.ts",
      [
        `--org=${provenanceOrg.orgId}`,
        "--production",
        `--source-invoice-lines=${sourcePath}`,
        `--out=${planPath}`,
      ],
    );
    if (plan.status !== 0) {
      throw new Error(
        `commercial provenance repair plan failed: ${plan.stderr || plan.stdout}`,
      );
    }
    const planned = JSON.parse(readFileSync(planPath, "utf8")) as any;
    if (
      planned.population.candidateLines !== 1 ||
      planned.population.blockers !== 0
    ) {
      throw new Error("commercial provenance preflight was not exact");
    }

    const wrongTenantActor = runRepair(
      "engine/src/validation/repair-invoice-line-commercial-provenance.ts",
      [
        `--org=${provenanceOrg.orgId}`,
        "--production",
        "--apply",
        `--actor=${untouchedActors.adminId}`,
        "--reason=Wrong-tenant actor must be rejected",
        `--source-invoice-lines=${sourcePath}`,
        `--out=${applyPath}`,
      ],
    );
    if (
      wrongTenantActor.status === 0 ||
      !`${wrongTenantActor.stderr}${wrongTenantActor.stdout}`.includes(
        "not an active user in the target organization",
      )
    ) {
      throw new Error("commercial repair accepted a cross-tenant actor");
    }

    await db.execute(
      sql.raw(`
        create or replace function parity_provenance_protected_mutation()
        returns trigger language plpgsql as $$
        begin
          if new.org_id = '${provenanceOrg.orgId}'::uuid
             and new.id = '${targetCandidate.lineId}'::uuid then
            new.amount := old.amount + 1;
          end if;
          return new;
        end
        $$;
        drop trigger if exists aa_parity_provenance_mutation on document_lines;
        create trigger aa_parity_provenance_mutation
        before update on document_lines
        for each row execute function parity_provenance_protected_mutation();
      `),
    );
    const protectedMutation = runRepair(
      "engine/src/validation/repair-invoice-line-commercial-provenance.ts",
      [
        `--org=${provenanceOrg.orgId}`,
        "--production",
        "--apply",
        `--actor=${provenanceActors.adminId}`,
        "--reason=Protected mutation rollback invariant",
        `--source-invoice-lines=${sourcePath}`,
        `--out=${applyPath}`,
      ],
    );
    await db.execute(sql.raw(`
      drop trigger if exists aa_parity_provenance_mutation on document_lines;
      drop function if exists parity_provenance_protected_mutation();
    `));
    if (
      protectedMutation.status === 0 ||
      !`${protectedMutation.stderr}${protectedMutation.stdout}`.includes(
        "protected document or GL state changed; rolling back",
      )
    ) {
      throw new Error(
        "commercial repair did not roll back a protected-field mutation",
      );
    }
    const rolledBack = await one<{
      ...
      select amount::text, quantity::text, item_id,
             custom->>'sourceLineRef' as source_ref
        from document_lines
       where org_id = ${provenanceOrg.orgId}
         and id = ${targetCandidate.lineId}
    `);
    if (
      rolledBack.amount !== "100.0000" ||
      rolledBack.quantity !== "1.00000000" ||
      rolledBack.item_id !== null ||
      rolledBack.source_ref !== null
    ) {
      throw new Error(
        `failed commercial repair left partial writes: ${JSON.stringify(rolledBack)}`,
      );
    }
    ...
    const apply = runRepair(
      ...
      [
        ...
        "--apply",
        `--actor=${provenanceActors.adminId}`,
        "--reason=Controller approved source commercial provenance restoration",
        `--source-invoice-lines=${sourcePath}`,
        `--out=${applyPath}`,
      ],
    );
    if (apply.status !== 0) {
      throw new Error(
        `commercial provenance repair apply failed: ${apply.stderr || apply.stdout}`,
      );
    }
    const applied = JSON.parse(readFileSync(applyPath, "utf8")) as any;
    const final = await one<{
      ...
      select line.amount::text, line.quantity::text, line.unit_price::text,
             line.item_id, line.custom->>'sourceLineRef' as source_ref,
             (
               select count(*)::int
                 from audit_log audit
                where audit.org_id = ${provenanceOrg.orgId}
                  and audit.table_name = 'document_lines'
                  and audit.row_id = ${targetCandidate.lineId}
                  and audit.changes->>'mode' =
                      'source_commercial_provenance_correction'
             ) as line_audits,
             (
               select count(*)::int
                 from audit_log audit
                where audit.org_id = ${provenanceOrg.orgId}
                  and audit.table_name = 'documents'
                  and audit.row_id = ${targetCandidate.documentId}
                  and audit.request_id like 'source-commercial-provenance:%'
             ) as document_audits
        from document_lines line
       where line.org_id = ${provenanceOrg.orgId}
         and line.id = ${targetCandidate.lineId}
    `);
    const untouched = await one<{
      ...
      select quantity::text, item_id,
             custom->>'sourceLineRef' as source_ref
        from document_lines
       where org_id = ${untouchedOrg.orgId}
         and id = ${untouchedCandidate.lineId}
    `);
    const commercialOk =
      applied.invariants.protectedDocumentAndGlChanged === 0 &&
      applied.invariants.updatedLines === 1 &&
      applied.invariants.auditedLines === 1 &&
      applied.invariants.auditedDocuments === 1 &&
      final.amount === "100.0000" &&
      final.quantity === "2.00000000" &&
      final.unit_price === "50.00000000" &&
      final.item_id === provenanceOrg.items.service &&
      final.source_ref === "L1" &&
      final.line_audits === 1 &&
      final.document_audits === 1 &&
      untouched.quantity === "1.00000000" &&
      untouched.item_id === null &&
      untouched.source_ref === null;
    saveJson(
      join(
        evidenceDir,
        `repair-commercial-provenance-${marker}.json`,
      ),
      {
        checkpoint: `repair-commercial-provenance-${marker}`,
        plan: planned.population,
        rejectedCrossTenantActor: true,
        protectedMutationRolledBack: true,
        rollbackState: rolledBack,
        apply: applied.invariants,
        final,
        untouchedTenant: untouched,
        comparison: { ok: commercialOk },
      },
    );
    if (!commercialOk) {
      throw new Error("commercial provenance repair invariants failed");
    }
    console.log(`PASS repair-commercial-provenance-${marker}`);
  } finally {
    await db.execute(sql.raw(`
      drop trigger if exists aa_parity_provenance_mutation on document_lines;
      drop function if exists parity_provenance_protected_mutation();
    `));
    await dropScratchOrg(provenanceOrg.orgId);
    await dropScratchOrg(untouchedOrg.orgId);
  }
}
...
try {
  switch (process.argv[2]) {
    ...
    case "run-sync-corrections":
      ...
      break;
    case "run-controlled-repairs":
      await runControlledRepairInvariants();
      break;
