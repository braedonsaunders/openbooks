...
import { CLOSE_MODULES, ensureCloseDefaults, periodScopeAdvisoryLock } from "../close/close.ts";
...
/** Persist leftover migrated item default cost through exact decimal then ledger money. Fail closed. */
function persistItemDefaultCost(value: unknown): string {
  const exact = canonicalDecimal(value, 4);
  if (exact === null) throw new Error("item default cost must be an exact decimal");
  try {
    return normalizeMoney(exact);
  } catch {
    throw new Error("item default cost must be an exact decimal");
  }
}
...
/** Persist leftover migrated time-entry cost rate through exact decimal then ledger money. Fail closed. */
function persistTimeEntryCostRate(value: unknown): string {
  const exact = canonicalDecimal(value, 4);
  if (exact === null) throw new Error("time-entry cost rate must be an exact decimal");
  try {
    return normalizeMoney(exact);
  } catch {
    throw new Error("time-entry cost rate must be an exact decimal");
  }
}
    ...
    if (prior) {
      ...
      const update = (tx: SqlExecutor) =>
        tx.execute(sql`update time_entries set worked_on=${str(f.workedOn) ?? "1970-01-01"}, hours=${persistTimeEntryHours(f.hours == null || f.hours === "" ? "0" : f.hours)},
          ...
          is_billable=${!!f.isBillable}, cost_rate=${f.costRate == null || f.costRate === "" ? null : persistTimeEntryCostRate(f.costRate)}, bill_rate=${f.billRate == null || f.billRate === "" ? null : persistTimeEntryBillRate(f.billRate)},
