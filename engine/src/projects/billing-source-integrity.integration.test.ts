...
async function fixture(run: (f: {
  ...
  // Fixture seeds under explicit bypass: importing ../../web/lib/documents.ts
  // above pulls in the web request-org resolver, which denies every unscoped
  // query. Product calls keep their own withOrgTransaction scope.
  ...
  try {
    const seeded = await withBypassContext(async () => {
      ...
      return { actor, approver, project, subcontract, sov };
    });
    ...
    assert.equal((await withOrgTransaction(org.orgId, () => submitAndReleaseIfUngated("vendor_bill", generated.vendorBillDocumentId, actor))).autoApproved, true);
    await withOrgTransaction(org.orgId, () => postDocument(generated.vendorBillDocumentId, { control: { ar: org.accounts.ar, ap: org.accounts.ap, bank: org.accounts.bank } }, { audit: { actorId: approver, source: "test" } }));
...
async function lines(f: Fixture, id: string) {
  return withOrgContext(f.org.orgId, async () =>
    (await db.execute<NonNullable<Parameters<typeof applyDocumentEdit>[2]["lines"]>[number] & Record<string, unknown>>(sql`
...
async function post(f: Fixture, id: string, kind = "vendor_bill") {
  assert.equal((await withOrgTransaction(f.org.orgId, () => submitAndReleaseIfUngated(kind, id, f.actor))).autoApproved, true);
  await withOrgTransaction(f.org.orgId, () => postDocument(id, { control: { ar: f.org.accounts.ar, ap: f.org.accounts.ap, bank: f.org.accounts.bank } }, { audit: { actorId: f.approver, source: "test" } }));
}
async function refusePost(f: Fixture, id: string, kind = "vendor_bill") {
  assert.equal((await withOrgTransaction(f.org.orgId, () => submitAndReleaseIfUngated(kind, id, f.actor))).autoApproved, true);
  const before = await snapshot(f, id);
  await assert.rejects(withOrgTransaction(f.org.orgId, () => postDocument(id, { control: { ar: f.org.accounts.ar, ap: f.org.accounts.ap, bank: f.org.accounts.bank } }, { audit: { actorId: f.approver, source: "test" } })), /source billing workflow/);
  assert.deepEqual(await snapshot(f, id), before);
}
...
for (const customer of [false, true]) {
  ...
  test(`${kind} posts with real before_post dimension assignments on its first attempt`, enabled, async () => fixture(async f => {
    ...
    const journal = await withOrgContext(f.org.orgId, async () =>
      (await db.execute<{ departmentId: string; locationId: string; classId: string }>(sql`
    ...
    const evidence = await withOrgContext(f.org.orgId, async () =>
      (await db.execute<{ status: string }>(sql`select status from script_runs
  ...
  test(`${kind} refuses before_post source project replacement and rolls back automation evidence`, enabled, async () => fixture(async f => {
    ...
    await withBypassContext(() => db.execute(sql`insert into projects(id,org_id,name,subsidiary_id,customer_id,status)
    ...
    const evidence = await withOrgContext(f.org.orgId, async () =>
      (await db.execute(sql`select id from script_runs
        where org_id=${f.org.orgId} and script_id=${scriptId} and target_id=${id}`)).rows);
...
for (const scenario of ["vendor release", "vendor application", "customer progress", "customer release"] as const) {
  test(`${scenario} protects source economics during edits and posts unchanged with metadata`, enabled, async () => fixture(async f => {
    ...
    assert.deepEqual((await snapshot(f, id))!.lines, originalSnapshot);
    const editedHeader = await withOrgContext(f.org.orgId, async () =>
      (await db.execute<{ memo: string; reference_number: string; due_date: string }>(sql`
    ...
    const doc = await withOrgContext(f.org.orgId, async () =>
      (await db.execute<{ status: string; memo: string; total: string }>(sql`select status,memo,total from documents where org_id=${f.org.orgId} and id=${id}`)).rows[0]!);
    ...
    const posted = await withOrgContext(f.org.orgId, async () =>
      (await db.execute<{ account_id: string; amount: string }>(sql`
...
for (const scenario of ["vendor release", "vendor application", "customer progress", "customer release"] as const) {
  test(`${scenario} rejects already-corrupted legacy drafts before posting writes`, enabled, async () => fixture(async f => {
    ...
    // setup writes land before the refusal under test.
    await withBypassContext(async () => {
...
test("retainage configuration change fails closed without remapping the generated release", enabled, async () => fixture(async f => {
  ...
  await withBypassContext(() => db.execute(sql`update orgs set settings=jsonb_set(settings,'{controlAccounts,retainagePayable}',to_jsonb(${f.org.accounts.cogs}::text)) where id=${f.org.orgId}`));
...
test("zero-retainage customer progress remains postable and custom markers cannot associate an ordinary bill", enabled, async () => fixture(async f => {
  ...
  // Scoped so the false is a real evaluation against the linked reservation,
  // not an empty unscoped read.
  assert.equal(await withOrgContext(f.org.orgId, () => assertGeneratedBillingEdit(db, randomUUID(), linked, { total: "999" }, null)), false);
}));
...
test("full retainage and multiple gross lines preserve exact source allocation", enabled, async () => fixture(async f => {
  ...
  await post(f, id, "customer_invoice");
  assert.equal((await withOrgContext(f.org.orgId, () => db.execute<{ total: string }>(sql`select total from documents where org_id=${f.org.orgId} and id=${id}`))).rows[0]!.total, "0.0000");
}));

test("explicit capitalization accounts remain valid on generated vendor bills", enabled, async () => fixture(async f => {
  await withBypassContext(() => db.execute(sql`update subcontract_sov_lines set expense_account_id=${f.org.accounts.taxInput} where org_id=${f.org.orgId} and id=${f.sov}`));
  const id = await vendorBill(f);
  await post(f, id);
  assert.equal((await withOrgContext(f.org.orgId, () => db.execute<{ amount: string }>(sql`select l.amount from journal_lines l join documents d on d.org_id=l.org_id and d.posted_entry_id=l.entry_id
