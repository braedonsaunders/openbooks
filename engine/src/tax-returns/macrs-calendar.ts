...
export function taxYearWindowEvidence(window: RegisteredMacrsYearWindow | TaxYearWindow): TaxYearWindowEvidence {
  return {
...
export function normalizeTaxYearWindowEvidence(evidence: readonly TaxYearWindowEvidence[]): TaxYearWindowEvidence[] {
  ...
  return [...distinct.values()].sort((left, right) => left.id.localeCompare(right.id));
}
