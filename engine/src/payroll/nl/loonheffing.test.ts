...
test("year resolution: 2026 computes, every other year throws by name", () => {
  ...
  assert.equal(r.annualWage, 11988n);
});
...
test("schijf boundary pair: L=38.880 prices schijf 1, L=38.934 prices schijf 2", () => {
  ...
  assert.equal(below.annualWage, 38880n);
  assert.equal(below.grossAnnual, 13899n);
  ...
  assert.equal(above.annualWage, 38934n);
  assert.equal(above.grossAnnual, 13919n);
...
test("schijf boundary pair at the top: L=78.408 prices schijf 2, L=78.462 prices schijf 3", () => {
  ...
  assert.equal(below.annualWage, 78408n);
  assert.equal(below.grossAnnual, 28745n);
  ...
  assert.equal(above.annualWage, 78462n);
  assert.equal(above.grossAnnual, 28769n);
...
test("AHK fully afgebouwd at € 78.426, ARK at € 132.920", () => {
  ...
  assert.equal(lastBefore.annualWage, 132894n);
  assert.equal(lastBefore.applied.ark, 2n);
  const firstPast = calculateNlStatutory({ income: "11079.00", periodsPerYear: 12, applyKorting: true, ...DECL });
  assert.equal(firstPast.annualWage, 132948n);
  assert.equal(firstPast.applied.ark, 0n);
});
...
test("capping order is AHK-first: low AOW wage leaves ARK € 666, low wage leaves ARK € 12", () => {
  ...
  assert.equal(low.applied.ark, 12n);
...
test("AOW-1945 schijf 1 runs to € 41.123 at 17,85%", () => {
  ...
  assert.equal(r.annualWage, 41094n);
  assert.equal(r.grossAnnual, 7335n);
  ...
  assert.equal(over.annualWage, 41148n);
  assert.equal(over.grossAnnual, 7349n);
});
...
test("Lmax golden and the above-Lmax systematiek-1 rule", () => {
  ...
  assert.equal(top.annualWage, 133110n);
  assert.equal(top.grossAnnual, 55820n);
  ...
  assert.equal(hi.annualWage, null);
