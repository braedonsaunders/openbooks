...
import { MS_CERTIFICATE, MS_REGION } from "./ms-declaration.ts";
import { MS_RATES_2026, MS_WITHHOLDING, msAnnualTax, msRoundToDollar } from "./ms.ts";
...
function cert(answers: Record<string, string> = {}): ResolvedCertificate {
  return resolveCertificate({
    ...
    stored: [{ certificateKey: MS_CERTIFICATE.key, answers, effectiveFrom: null }],
  });
}
