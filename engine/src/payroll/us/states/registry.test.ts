...
test("the pack now withholds state income tax somewhere", () => {
  ...
  assert.deepEqual(
    implementedUsStates(),
    ["AL", "AZ", "CA", "CO", "CT", "DE", "GA", "IL", "IN", "IA", "KY", "MD", "MA", "MI", "MN", "NJ", "NY", "NC", "OH", "OR", "PA", "SC", "UT", "VA", "WV", "WI"],
  );
...
test("supported states are the implemented ones PLUS the genuinely no-tax ones", () => {
  ...
  assert.equal(supported.length, 35); // 26 implemented + 9 no-tax
  for (const state of ["CA", "CO", "CT", "DE", "NY", "PA", "IL", "NJ", "OH", "MI", "MA", "MD", "GA", "NC",
    "AL", "AZ", "IN", "KY", "VA", "WV", "IA", "MN", "WI", "UT", "OR", "SC",
    "TX", "FL", "WA"]) {
...
test("an untranscribed state is refused BY NAME, with the publication and the file", () => {
  // Alabama and South Carolina now compute. Arkansas carries the refusal:
  // the list of states this sentence can be written about is supposed to
  // keep shrinking.
  assert.throws(
    () => requireUsStateWithholding("AR"),
    (error: unknown) => {
      const message = (error as Error).message;
      assert.match(message, /AR income tax withholding is not implemented/);
      assert.match(message, /Arkansas Withholding Tax Formula/);
      assert.match(message, /engine\/src\/payroll\/us\/states\/ar\.ts/);
      assert.match(message, /withholding the federal amount.*would each be silently\s+wrong/s);
      assert.match(message, /Implemented today: AL, AZ, CA, CO, CT, DE, GA, IL, IN, IA, KY, MD, MA, MI, MN, NJ, NY, NC, OH, OR, PA, SC, UT, VA, WV, WI/);
      return true;
    },
  );
});
...
test("an Indiana resident working in Ohio is withheld INDIANA, both engines present", () => {
  const resolved = resolveWithholding({
    country: "US", workRegion: "OH", residenceRegion: "IN",
    certificatesOnFile: ["us_oh_it4"],
  });
  const regions = resolved.levies.filter((levy) => levy.level === "region");
  assert.deepEqual(regions.map((levy) => levy.region), ["IN"]);
...
test("Iowa, Wisconsin, Minnesota, Kentucky, Virginia, West Virginia and Maryland name their work-side partners", () => {
  ...
  assert.equal(reciprocityAgreement("US", "WI", "IL")!.certificateKey, "us_wi_w220");
  assert.deepEqual(reciprocityPartners("US", "MN").sort(), ["MI", "ND"]);
  assert.equal(reciprocityAgreement("US", "MN", "MI")!.certificateKey, "us_mn_mwr");
  assert.deepEqual(reciprocityPartners("US", "KY").sort(), ["IL", "IN", "MI", "WI", "WV"]);
  assert.equal(reciprocityAgreement("US", "KY", "VA"), null);
  assert.equal(reciprocityAgreement("US", "KY", "OH"), null);
...
test("Ohio reciprocity relieves the STATE, and leaves the city standing", () => {
  ...
  const regions = resolved.levies.filter((levy) => levy.level === "region");
  assert.deepEqual(regions.map((levy) => levy.region), ["KY"]);
  assert.equal(regions[0]!.basis, "reciprocity");
...
test("an employee with no residence recorded resolves exactly as before", () => {
  ...
  for (const state of [
    "CA", "CO", "CT", "DE", "NY", "PA", "IL", "NJ", "OH", "MI", "MA", "MD", "GA", "NC",
    "AL", "AZ", "IN", "KY", "VA", "WV", "IA", "MN", "WI", "UT", "OR", "SC",
  ]) {
