...
test("UT Example 1 — weekly $400, Single: $12 (Rev. 4/26 p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-06-05", periodsPerYear: 52, wages: "400.00", basis: "resident",
...
test("UT Example 3 — semimonthly $1,200, Married: $18 (Rev. 4/26 p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-06-15", periodsPerYear: 24, wages: "1200.00", basis: "resident",
...
test("UT Example 4 — monthly $7,800, Married: $347 (Rev. 4/26 p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-06-30", periodsPerYear: 12, wages: "7800.00", basis: "resident",
    certificate: cert({ filing_status: "married" }),
  });
...
test("UT Example 5 — quarterly $9,000, Single: $367 (Rev. 4/26 p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-06-30", periodsPerYear: 4, wages: "9000.00", basis: "resident",
    certificate: cert({ filing_status: "single" }),
  });
...
test("UT Example 6 — daily $175, Married: $5 (Rev. 4/26 p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-06-05", periodsPerYear: 260, wages: "175.00", basis: "resident",
    certificate: cert({ filing_status: "married" }),
  });
...
test("UT Rev. 4/25 Example 2 — biweekly $2,600, Single: $117 (p. 11)", () => {
  ...
  const result = UT_WITHHOLDING.compute({
    payDate: "2026-03-13", periodsPerYear: 26, wages: "2600.00", basis: "resident",
    certificate: cert({ filing_status: "single" }),
  });
...
test("UT mid-year cut is the pay date, not a blended rate", () => {
  // Pub 14 Rev. 4/26: tables apply to pay periods beginning on or after
  // June 1, 2026. Without a period-start field this engine keys on pay date,
  // the same way Georgia keys its May 11, 2026 cut.
  const may = UT_WITHHOLDING.compute({
    payDate: "2026-05-29", periodsPerYear: 26, wages: "2600.00", basis: "resident",
    certificate: cert({ filing_status: "single" }),
  });
  const june = UT_WITHHOLDING.compute({
    payDate: "2026-06-01", periodsPerYear: 26, wages: "2600.00", basis: "resident",
    certificate: cert({ filing_status: "single" }),
  });
...
test("UT refuses a pay frequency it prints no schedule for", () => {
  assert.throws(
    () => UT_WITHHOLDING.compute({
      payDate: "2026-06-05", periodsPerYear: 13, wages: "2000", basis: "resident",
      certificate: cert(),
    }),
