...
import { worklistGates } from "../flows/gates.ts";
import {
  createScratchOrg,
  createScratchUser,
  dropScratchOrg,
...
async function makeFixture(): Promise<Fixture> {
  return await withBypass(async () => {
    ...
    const requesterId = await createScratchUser(org.orgId, "Module Requester", "admin");
    const approverId = await createScratchUser(org.orgId, "Module Approver", "admin");
    const outsiderId = await createScratchUser(org.orgId, "Module Outsider", "viewer");
...
function manifest(overrides: Record<string, unknown> = {}) {
  return {
    ...
    contributions: [{ kind: "page", route: "/lifecycle/widget", spec: specFor("/lifecycle/widget") }],
    ...overrides,
  };
}

type ApprovalOpts = {
  assignees?: { type: "user"; userId: string }[] | { type: "role"; role: string }[];
  reason?: string;
...
function installRequest(f: Fixture, m: Record<string, unknown>, opts: ApprovalOpts = {}) {
  return requestModuleInstallApproval({
    orgId: f.orgId,
    requesterId: f.requesterId,
    manifest: m,
    installerEffectivePermissions: opts.installerEffectivePermissions ?? ["records.read", "records.write"],
    ...(opts.grantedPermissions !== undefined ? { grantedPermissions: opts.grantedPermissions } : {}),
    assignees: (opts.assignees ?? [{ type: "user" as const, userId: f.approverId }]) as {
      type: "user";
      userId: string;
    }[],
    ...(opts.quorum !== undefined ? { quorum: opts.quorum } : {}),
    ...(opts.signatureRequired !== undefined ? { signatureRequired: opts.signatureRequired } : {}),
...
async function moduleRow(orgId: string, key: string) {
  const rows = (
    await withOrgContext(orgId, () =>
      db.execute<{ id: string; status: string; active_version_id: string | null; granted_permissions: string[] }>(sql`
...
async function gateRowsForModule(orgId: string, moduleId: string) {
  return (
    await withOrgContext(orgId, () =>
      db.execute<{ id: string; status: string; subject_kind: string; decided_by: string | null }>(sql`
...
type AuditRow = {
  ...
  row_id: string;
  action: string;
  changes: { reason?: unknown; before?: unknown; after?: unknown; event?: unknown };
...
test("install proposal creates a gate, approval activates the version", { skip: !DB }, async () => {
  ...
  try {
    ...
    const gates = await withBypass(() => gateRowsForModule(f.orgId, req.moduleId));
    assert.equal(gates.length, 1);
    assert.equal(gates[0]!.status, "pending");
    assert.equal(gates[0]!.subject_kind, "module_version");
    assert.equal(await liveProjections(f.orgId, req.moduleId).then((r) => r.length), 0);

    // The approver sees it in the EXISTING approval worklist.
    const worklist = await withBypass(() => worklistGates(f.orgId, f.approverId));
    assert.ok(
    ...
    const mod = await withBypass(() => moduleRow(f.orgId, "lifecycle-widget"));
    assert.equal(mod!.active_version_id, decision.versionId);
    const versions = await withBypass(() => versionRows(f.orgId, req.moduleId));
    assert.deepEqual(
    ...
    const moduleAudit = await withBypass(() => auditFor(f.orgId, "modules", req.moduleId));
    const byEvent = new Map(moduleAudit.map((r) => [r.changes.event as string, r]));
    assertAudited(
      moduleAudit.filter((r) =>
        ["module_install_staged", "module_install_approval_requested"].includes(r.changes.event as string),
      ),
      f.requesterId,
    );
    assertAudited(
      moduleAudit.filter(
        (r) => !["module_install_staged", "module_install_approval_requested"].includes(r.changes.event as string),
      ),
      f.approverId,
    );
    assert.ok(byEvent.has("module_install_staged"), "staging is audited");
    assert.ok(byEvent.has("module_install_approval_requested"), "the approval request is audited");
    const versionAudit = await withBypass(() => auditFor(f.orgId, "module_versions", decision.versionId!));
...
test("denial leaves the proposal pending and projects nothing", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const req = await withBypass(() => installRequest(f, manifest()));
    const decision = await withBypass(() =>
    ...
    assert.deepEqual(await withBypass(() => versionRows(f.orgId, req.moduleId)), []);
    assert.deepEqual(await withBypass(() => liveProjections(f.orgId, req.moduleId)), []);

    const gates = await withBypass(() => gateRowsForModule(f.orgId, req.moduleId));
    assert.equal(gates[0]!.status, "rejected");

    const moduleAudit = await withBypass(() => auditFor(f.orgId, "modules", req.moduleId));
    const denial = moduleAudit.find((r) => (r.changes.event as string) === "module_approval_denied");
...
test("the requester cannot approve their own proposal", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const req = await withBypass(() => installRequest(f, manifest()));
    await assert.rejects(
      withBypass(() =>
        decideModuleApproval({ gateId: req.gateIds[0]!, decision: "approved", userId: f.requesterId }),
      ),
      (error: unknown) =>
        error instanceof ModuleLifecycleError && /own module/.test(error.message),
      "self-approval is refused before the gate is touched",
    );
    const gates = await withBypass(() => gateRowsForModule(f.orgId, req.moduleId));
    assert.equal(gates[0]!.status, "pending", "refused decision leaves the gate pending");
...
test("a non-approver cannot decide a module gate", { skip: !DB }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      withBypass(() =>
        decideModuleApproval({ gateId: req.gateIds[0]!, decision: "approved", userId: f.outsiderId }),
      ),
...
test("a decided gate cannot be decided twice", { skip: !DB }, async () => {
  ...
  try {
    ...
    const live = await withBypass(() => liveProjections(f.orgId, req.moduleId));
    assert.equal(live.length, 1, "no duplicate projection from the refused replay");
  } finally {
    await dropFixture(f);
  }
});

test("requesting the same proposal twice replays the pending gate", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    ...
    const gates = await withBypass(() => gateRowsForModule(f.orgId, first.moduleId));
...
test("upgrade with an added capability requires a gate; approval supersedes", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const first = await withBypass(() => installRequest(f, manifest()));
    const v1 = await withBypass(() =>
      decideModuleApproval({ gateId: first.gateIds[0]!, decision: "approved", userId: f.approverId }),
    );
    ...
    const v2manifest = manifest({
      ...
      contributions: [
        { kind: "page", route: "/lifecycle/widget", spec: specFor("/lifecycle/widget") },
        { kind: "page", route: "/lifecycle/extra", spec: specFor("/lifecycle/extra") },
      ],
    });
    ...
    assert.deepEqual(req.addsCapabilities, ["records.create"], "added capability is reported for the approver");
    ...
    const live = await withBypass(() => liveProjections(f.orgId, req.moduleId));
    assert.deepEqual(
      live.map((r) => r.route).sort(),
      ["/lifecycle/extra", "/lifecycle/widget"],
    );
    const mod = await withBypass(() => moduleRow(f.orgId, "lifecycle-widget"));
    assert.deepEqual(mod!.granted_permissions, ["records.create", "records.read"]);
  } finally {
    await dropFixture(f);
  }
});

test("install while a version is active is refused; upgrade of an unknown module is refused", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const req = await withBypass(() => installRequest(f, manifest()));
    await withBypass(() =>
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof ModuleLifecycleError && /upgrade/.test(error.message),
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof ModuleLifecycleError && /not installed/.test(error.message),
...
test("deactivation withdraws projections; reactivation restores them", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const req = await withBypass(() => installRequest(f, manifest()));
    await withBypass(() =>
    ...
    const deactivated = await withBypass(() =>
      deactivateModule({ orgId: f.orgId, actorId: f.approverId, key: "lifecycle-widget", reason: "taking down" }),
    );
    assert.ok(deactivated.moduleId);
    const mod = await withBypass(() => moduleRow(f.orgId, "lifecycle-widget"));
    assert.equal(mod!.status, "disabled");
    assert.deepEqual(await withBypass(() => liveProjections(f.orgId, req.moduleId)), []);
    const uninstallAudit = await withBypass(() => auditFor(f.orgId, "modules", req.moduleId));
    assert.ok(
    ...
    await withBypass(() =>
      reactivateModule({ orgId: f.orgId, actorId: f.approverId, key: "lifecycle-widget", reason: "back up" }),
    );
    const revived = await withBypass(() => moduleRow(f.orgId, "lifecycle-widget"));
...
test("a superseded version can be marked rolled back; the active one cannot", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const first = await withBypass(() => installRequest(f, manifest()));
    await withBypass(() =>
    ...
    const rollbackAudit = await withBypass(() => auditFor(f.orgId, "module_versions", v1.id));
    assertAudited(rollbackAudit, f.approverId);

    await assert.rejects(
      withBypass(() =>
        markVersionRolledBack({ orgId: f.orgId, actorId: f.approverId, versionId: v2.versionId! }),
      ),
      (error: unknown) => error instanceof ModuleLifecycleError && /active/.test(error.message),
...
test("a pending request can be cancelled; a signature gate needs its signature", { skip: !DB }, async () => {
  const f = await makeFixture();
  try {
    const req = await withBypass(() => installRequest(f, manifest()));
    ...
    const signed = await withBypass(() =>
      installRequest(f, manifest({ key: "signed-widget", name: "Signed Widget" }), {
        signatureRequired: true,
      }),
    );
    await assert.rejects(
      withBypass(() =>
        decideModuleApproval({ gateId: signed.gateIds[0]!, decision: "approved", userId: f.approverId }),
      ),
