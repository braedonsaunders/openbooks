...
test('module drawer installs, upgrades, renders, and rolls back a page contribution', async ({ browser, baseURL }) => {
  ...
  try {
    ...
    await page.goto('/admin/modules')
    await page.getByRole('link', { name: 'New extension', exact: true }).click()
    ...
    await page.getByRole('link', { name: 'Import an existing page-customization package' }).click()
    const drawer = page.getByRole('dialog')
    await drawer.locator('summary').filter({ hasText: /^Advanced package source$/ }).click()
