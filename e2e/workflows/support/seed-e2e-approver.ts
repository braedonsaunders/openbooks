...
async function main(): Promise<void> {
  ...
  // Explicit connect: pg's implicit first-query connect never settles under
  // the tsx loader (the awaited query drains silently), in CI as well as here.
  ...
  try {
    // One transaction for the writes: the active-user role trigger (0096)
    // refuses a freshly inserted active user with no roles yet, so the user
    // and both grants must commit atomically — same as seedAdmin.
    ...
    const userId =
      created.rows[0]?.id ??
      (
        await client.query<{ id: string }>("select id from users where org_id = $1 and email = $2", [
          orgId,
          email,
        ])
      ).rows[0]!.id;
