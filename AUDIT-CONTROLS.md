...
As of this revision the schema carries **182 triggers**, **151 PL/pgSQL guard
functions**, **387 row-level-security policies**, and row-level security
enabled on **378 tables**.
...
| G10 | `verify:release` runs repository-artifact, dependency, container-security, and explicit-any checks; lint; end-to-end and workspace type checking; the full `npm test` suite; and a production web build. `verify:release:quick` is the same gate without the suite, for the fast path; neither command invokes the browser end-to-end suite. | `package.json`: `npm run verify:release`, `npm run verify:release:quick` |
...
| G15 | The golden harness produces a diffable checkpoint — counts, trial balance, control tie-outs, check results, and report timings — that can be compared across commits to detect an unintended change in reported figures. | `engine/src/golden/scenario.ts` |
