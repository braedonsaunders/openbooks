 ...
 * Usage: node --import tsx scripts/payroll-reconcile-filing-accounts.ts
 *   --org UUID --actor UUID --input /absolute/reviewed-rows.json [--apply]
 * Defaults to validation with rollback. Rows: {stubId, filingAccountId, reason, reference}[].
...
async function main() {
  ...
  try {
    await withOrgTransaction(values.org, async () => {
      count = await reconcilePayrollFilingAccounts({
        orgId: values.org!,
        actorId: values.actor!,
        rows: rows as PayrollFilingReconciliation[],
      });
  ...
  console.log(
    `${values.apply ? "Reconciled and audited" : "Validated and rolled back"} ${count} pay stubs.`,
  );
}
void main()
  .catch((error: unknown) => {
